...
## 🔄 Modifica servicios del `compose.yml` para obtener imágenes desde `Docker Hub`

En nuestro archivo `compose.yml` los servicios `s-user-service` y `s-course-service` estaban usando los bloques `build`
para construir sus imágenes a partir de un `Dockerfile`, es decir, nuestro `compose.yml` actuaba como una herramienta
de desarrollo, construyendo las imágenes en cada despliegue.

Veamos un extracto del servicio `s-user-service` para recordar cómo es que estábamos construyendo la imagen.
...
En este apartado, en vez de dejar que `Docker Compose` construya las imágenes de nuestros servicios de negocio, lo que
haremos será obtener las imágenes de forma remota desde `Docker Hub`, similar a cómo obtenemos las imágenes de `mysql`
y `postgres`. Por lo tanto, refactorizaremos nuestro archivo `compose.yml` para que se comporte como un orquestador
de despliegue puro, consumiendo imágenes remotas de `Docker Hub`.

Primero eliminaremos las imágenes de los microservicios de negocio (como lo hicimos en el apartado anterior)
y nos quedaremos únicamente con las imágenes de las bases de datos.
...
- Como ahora traeremos las imágenes desde `Docker Hub` ya no necesitamos usar el bloque `build` que teníamos antes, por
  eso lo eliminé.
- En `image` especificamos la imagen que vamos a usar. Lo que hará Docker será primero buscar la imagen en
  nuestra máquina local, al no encontrarlo irá a `Docker Hub` a bajarlo. Es obligatorio incluir el prefijo `magadiflo/`
  para que Docker sepa que debe buscar en tu repositorio personal y no en las imágenes oficiales.

### 2. Despliegue con "Lazy Loading" Automático

Al ejecutar el despliegue en un entorno limpio (sin las imágenes de los servicios), `Docker Compose` activa su
...
[+] up 21/21                                                                                                    
 ✔ Image magadiflo/course-service:latest Pulled                                                                 
 ✔ Image magadiflo/user-service:latest   Pulled                                                                 
...
- **Status Pulled:** La salida de consola confirma que Docker descargó las imágenes `(Image ... Pulled)` en lugar de
  construirlas.
- **Eficiencia:** Docker solo descargará las capas que no existan previamente en tu host local.
- **Resultado:** Los contenedores `c-user-service` y `c-course-service` ahora corren basados en la imagen `remota`,
  no en la imagen local anterior.
...
1f50b4b1103d   magadiflo/user-service:latest     "java org.springfram…"   3 minutes ago   Up 3 minutes             0.0.0.0:8001->8001/tcp, [::]:8001->8001/tcp   c-user-service
18ce3d183042   magadiflo/course-service:latest   "java org.springfram…"   3 minutes ago   Up 3 minutes             0.0.0.0:8002->8002/tcp, [::]:8002->8002/tcp   c-course-service
a1b675694303   postgres:17-alpine                "docker-entrypoint.s…"   3 minutes ago   Up 3 minutes (healthy)   0.0.0.0:5433->5432/tcp, [::]:5433->5432/tcp   c-postgres
057b4ba803b4   mysql:8.0.41-debian               "docker-entrypoint.s…"   3 minutes ago   Up 3 minutes (healthy)   0.0.0.0:3307->3306/tcp, [::]:3307->3306/tcp   c-mysql
````

## 🧪 Validación de Integración Final

Realizamos la prueba reina: consultar el microservicio de cursos con carga de relaciones.
...
< Date: Tue, 14 Apr 2026 22:34:16 GMT
<
{
  "id": 3,
  "name": "Spring WebFlux",
...
> **Prueba Superada:**  
> El JSON devuelto con los datos de "Lesly" (provenientes del `user-service` en `MySQL`) confirma que la red de Docker,
> las variables de entorno y la comunicación entre contenedores siguen funcionando a la perfección,
> independientemente del origen de la imagen.
