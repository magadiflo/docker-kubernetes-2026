...
>
> Para lograr exactamente lo mismo (que Kubernetes descargue e inyecte la nueva imagen `:latest` desde Docker Hub)
> sin apagar tus aplicaciones ni un solo segundo, la norma estándar de la industria es forzar un reinicio controlado
> mediante una estrategia de `Rolling Update` utilizando el comando `kubectl rollout restart`. Esto le instruye
> a Kubernetes a levantar los nuevos Pods actualizados de forma progresiva, y **no destruirá los Pods viejos hasta
> que las nuevas réplicas pasen exitosamente sus pruebas de salud** (*Health Check*).
>
> Para ejecutar esta acción sobre múltiples recursos en línea, Kubernetes ofrece total flexibilidad sintáctica
> en la terminal. Elige la variante que mejor se adapte a tu flujo de trabajo:
>
> 📂 **Opción 1: Sintaxis con Barra (`/`).** Ideal si prefieres el formato nativo `<tipo>/<nombre>` encadenando cada
> recurso separado por un espacio:
> ````bash
> $ kubectl rollout restart deployment/d-user-service deployment/d-course-service
> ````
>
> 🔀 **Opción 2: Sintaxis Separada por Espacios.** Indicas el tipo de recurso al inicio y simplemente listas los
> nombres correspondientes al final:
> ````bash
> $ kubectl rollout restart deployment d-user-service d-course-service
> ````
>
> ⚡ **Opción 3: Automatización Masiva (--all).** Si te encuentras en un laboratorio y deseas actualizar absolutamente
> todos los deployments activos de tu namespace actual en una sola línea, usa el selector masivo:
> ````bash
...
## 🧪 Probando el Perfil Activo Definido en el ConfigMap

### 🔗 Obteniendo la URL del servicio de usuarios

Levantamos el servicio `svc-user-service` para poder acceder desde fuera de `Minikube` y realizar una petición a
nuestra aplicación:
...
### 📡 Realizando petición al endpoint `/info`

````bash
$ curl -v http://127.0.0.1:52569/api/v1/users/info | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Sun, 05 Jul 2026 00:47:38 GMT
<
{
  "POD_NAME": "d-user-service-5588cd6d77-95pw5",
  "config_text": "Configuraciones del perfil activo [dev] desde [user-service-config-map.yml]",
  "POD_IP": "10.244.1.19",
  "users": [
    {
      "id": 1,
...
|---------------|----------------------------------------------|-----------------------------------------------------------------------|
| `config_text` | `Configuraciones del perfil activo [dev]...` | ✅ Spring Boot cargó el bloque `user-service-dev.yml` del `ConfigMap`. |
| `POD_NAME`    | `d-user-service-5588cd6d77-95pw5`            | ✅ Inyectado correctamente por la `Downward API`.                      |
| `POD_IP`      | `10.244.1.19`                                | ✅ IP interna del Pod dentro de la red del clúster.                    |

El campo `config_text` es la evidencia clave: su valor corresponde exactamente al definido en el bloque
`user-service-dev.yml` del `ConfigMap`, confirmando que:

1. La variable `SPRING_PROFILES_ACTIVE=dev` fue inyectada correctamente desde el `ConfigMap`.
2. Spring Boot activó el perfil `dev` y cargó el bloque `user-service-dev.yml` como fuente de propiedades.
3. La propiedad `config.text` fue sobreescrita por el valor del perfil activo, en lugar del valor base definido en
   `user-service.yml`.

## 🔄 Probando el Cambio de Perfil a `prod` en usuarios

### ✏️ Paso 1: Modificando el ConfigMap

Actualizamos la clave `spring_profiles_active` para apuntar al perfil `prod`:
...
data:
  ...
  db_name: db_user_service
  k8s_course_service_name: svc-course-service
  spring_profiles_active: prod                  # ← cambiado de dev a prod
  user-service.yml: |-
    config:
      text: Configuraciones del perfil activo [default] desde [user-service-config-map.yml]
  user-service-dev.yml: |-
...
### 📝 Paso 2: Aplicando el ConfigMap actualizado
...
### 🔁 Paso 3: Reiniciando el Deployment

En este escenario, el archivo `user-service-deployment.yml` **no sufrió ningún cambio**, por lo que `kubectl apply`
respondería con `unchanged` y no recrearía los Pods. Para forzar que los Pods lean la nueva configuración del
`ConfigMap`, usamos el reinicio controlado:
...
| `rollout`                   | Subcomando que gestiona el ciclo de despliegue de las aplicaciones.                                                     |
| `restart`                   | Fuerza un nuevo ciclo de despliegue, terminando los Pods existentes y creando nuevos con la configuración más reciente. |
| `deployment/d-user-service` | Tipo y nombre del recurso a reiniciar.                                                                                  |

#### 💡 `kubectl rollout restart` vs `kubectl delete` + `kubectl apply`

| Método                             | Comportamiento                                                                                     | ¿Recomendado?                 |
|------------------------------------|----------------------------------------------------------------------------------------------------|-------------------------------|
| `kubectl rollout restart`          | Reinicio **gradual** — termina y crea Pods de forma progresiva, manteniendo disponibilidad.        | ✅ Sí — estándar en producción |
| `kubectl delete` + `kubectl apply` | Elimina **todos** los Pods a la vez antes de crear los nuevos. Puede causar tiempo de inactividad. | ⚠️ Solo en desarrollo         |

### 🔍 Paso 4: Verificando los nuevos Pods

````bash
$ kubectl get pods
NAME                                READY   STATUS    RESTARTS       AGE
d-course-service-5b9bd8c648-dqkfh   1/1     Running   3 (19m ago)    2d1h
d-mysql-6989d9c76-pqh5m             1/1     Running   14 (19m ago)   9d
d-postgres-6f8f76ff47-vxpt9         1/1     Running   15 (19m ago)   9d
d-user-service-798588c6f5-cwvgw     1/1     Running   0              2m41s
d-user-service-798588c6f5-mwzfk     1/1     Running   0              2m37s
d-user-service-798588c6f5-sppwq     1/1     Running   0              2m44s
````

Los tres Pods de `user-service` fueron recreados con el nuevo hash `798588c6f5` y `RESTARTS: 0`, confirmando que
arrancaron desde cero con el perfil `prod` activo. ✅

### 🧪 Paso 5: Probando el perfil `prod`
...
Realizamos la petición al endpoint `/info`:
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Sun, 05 Jul 2026 01:03:25 GMT
<
{
  "POD_IP": "10.244.1.27",
  "users": [
  ...
  "POD_NAME": "d-user-service-798588c6f5-cwvgw",
  "config_text": "Configuraciones del perfil activo [prod] desde [user-service-config-map.yml]"
}
````

✅ El campo `config_text` confirma que Spring Boot cargó correctamente el bloque `user-service-prod.yml` del
`ConfigMap`, demostrando que el cambio de perfil de `dev` a `prod` fue aplicado exitosamente **sin necesidad de
reconstruir la imagen Docker**.

> 📌 **Nota:** Para continuar con los siguientes apartados, revertiremos el perfil a `dev` ejecutando los mismos
> pasos pero cambiando `spring_profiles_active: prod` por `spring_profiles_active: dev`. No documentaremos este
> proceso, ya que es idéntico al que acabamos de realizar.

## 📘 Configurando Liveness y Readiness en Microservicios con Spring Boot Actuator

En esta lección aprenderemos cómo Kubernetes verifica la salud de los Pods y contenedores mediante los llamados
**probes** (sondeos).

Estos chequeos los ejecuta el proceso `Kubelet`, que monitorea continuamente la salud de los contenedores y
notifica al `API Server` del `Control Plane` cuando algo no funciona correctamente. En base a esto, Kubernetes puede
reiniciar el contenedor o reemplazar el Pod defectuoso.

### 🔍 Tipos de Probes

| Probe                 | ¿Qué verifica?                                                                                                                            | ¿Qué hace Kubernetes si falla?                     |
|-----------------------|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------|
| 🔵 **LivenessProbe**  | Si el contenedor está funcionando correctamente o ha entrado en estado *broken*.                                                          | **Reinicia el contenedor.**                        |
| 🟢 **ReadinessProbe** | Si la aplicación está lista para aceptar tráfico entrante. Puede no estarlo porque aún está arrancando o está sobrecargada temporalmente. | **Deja de enviarle tráfico**, pero no lo reinicia. |

> 💡 **Regla mnemotécnica:**
> - `liveness` → ¿La aplicación sigue **viva**?
> - `readiness` → ¿La aplicación puede **recibir solicitudes**?

> 🧠 **Regla de Oro:** Un contenedor puede fallar el `Readiness` (porque se está conectando a la base de datos o
> está sobrecargado) pero pasar el `Liveness` (sigue vivo). En ese caso, Kubernetes lo deja tranquilo hasta que se
> recupere, sin matarlo.

### ⚡ Activando los Endpoints con Spring Boot Actuator

Para implementar los probes en Spring Boot aprovecharemos el `Spring Boot Actuator`, que ya está incluido en el
`pom.xml` de nuestros microservicios:
...
> 📌 En esta lección todas las configuraciones se realizarán únicamente en el microservicio `user-service`.
> Aunque el mismo procedimiento podría aplicarse al `course-service`, no efectuaremos ninguna modificación en él,
> ya que, por fines didácticos, nos centraremos exclusivamente en `user-service`.

### ⚙️ Configuración en `application.yml`

Agregamos la configuración de `Spring Boot Actuator` en el `application.yml` del microservicio `user-service`:
...
|----------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
| `endpoints.web.exposure.include: '*'`  | Expone todos los endpoints de Actuator vía HTTP. ⚠️ En producción, se recomienda restringir solo a los necesarios (ej. `health,info`).    |
| `endpoint.health.show-details: always` | El endpoint `/actuator/health` muestra información detallada del estado — base de datos, disco, etc. — en lugar de solo `"status": "UP"`. |
| `endpoint.health.probes.enabled: true` | Activa explícitamente los probes (`liveness` y `readiness`) dentro del endpoint `/actuator/health`.                                       |
| `health.livenessstate.enabled: true`   | Habilita el `liveness probe` — indica si la aplicación está viva o debe reiniciarse.                                                      |
| `health.readinessstate.enabled: true`  | Habilita el `readiness probe` — indica si la aplicación está lista para recibir tráfico.                                                  |

### 📡 Endpoints de Salud Generados

Con esta configuración guardada en el `application.yml`, **Spring Boot Actuator** mapeará y expondrá automáticamente
las rutas de diagnóstico. Sin embargo, para probarlas o entender cómo se consumen, debemos separar el mundo de la
infraestructura de nuestro entorno local:

- 🐳 **Rutas de Infraestructura (Las que consulta Kubernetes internamente):**  
  El proceso agente `Kubelet` vive dentro de la red del clúster, por lo que audita la salud pegándole directamente
  a la IP privada de cada Pod en su puerto nativo (`8001`):
    - `http://<pod-ip>:8001/actuator/health/liveness`
    - `http://<pod-ip>:8001/actuator/health/readiness`


- 💻 **Rutas de Desarrollo (Las que usas tú con curl desde Windows):**  
  Como tu terminal de Windows no puede acceder directamente a las IPs privadas de los Pods (`10.244.x.x`),
  debes usar el puente de red que te genera el comando `minikube service svc-user-service --url` para simular las
  peticiones:
    - `http://127.0.0.1:<puerto-minikube>/actuator/health/liveness`
    - `http://127.0.0.1:<puerto-minikube>/actuator/health/readiness`

### 🐳 Reconstruyendo y Publicando la Imagen

Como modificamos el `application.yml` del `user-service`, necesitamos reconstruir la imagen para que los cambios se
reflejen dentro del contenedor:

````bash
D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes-2026 (feature/section-17)
$ docker image build -t magadiflo/user-service .\business-domain\user-service                                   
[+] Building 29.1s (24/24) FINISHED
...                                                                          
````

Una vez construida localmente, la publicamos en `Docker Hub`:
...
````
[ Modificación de Código ] ──► [ Build & Push Docker ] ──► [ Imagen :latest en la Nube ]
                                                                      │
[ Próximo Paso ] ◄── Configurar los Probes dentro del Deployment ◄────┘
````

## 🏗️ Configurando el Despliegue con Liveness y Readiness Probes

Una vez que nuestra imagen Docker tiene el motor de Actuator encendido, el siguiente paso es instruir al orquestador
para que use esos endpoints. Editamos el manifiesto de despliegue `user-service-deployment.yml` e inyectamos los
bloques de sondeo dentro de la especificación de nuestro contenedor.
    ...
    spec:
      containers:
        - image: magadiflo/user-service:latest
          ...
          # 🟢 Sondeo de Disponibilidad (¿Puede recibir tráfico?)
          readinessProbe:
            httpGet:
          ...
          # 🔵 Sondeo de Vitalidad (¿La aplicación sigue viva?)
          livenessProbe:
            httpGet:
            ...
            periodSeconds: 10
            timeoutSeconds: 5
# Variable de entorno y demás configuraciones...
````
...
### 🚀 Aplicando Configuraciones y Auditoría del Clúster

Enviamos el manifiesto actualizado hacia el API Server de Kubernetes:
...
Si consultamos de inmediato el estado global del Deployment de usuarios vemos que no ha cambiado:

````bash
$ kubectl get deployments
NAME               READY   UP-TO-DATE   AVAILABLE   AGE
d-course-service   1/1     1            1           5d23h
d-mysql            1/1     1            1           12d
d-postgres         1/1     1            1           12d
d-user-service     3/3     1            3           5d23h
````

> 🧠 **¿Por qué el Deployment `d-user-service` se mantiene estructuralmente tal cual?**  
> El recurso `Deployment` es simplemente el administrador de alto nivel. Él no cambia sus metadatos, selectores
> ni el número total de réplicas solicitadas (siguen siendo 3). Sin embargo, como el cambio ocurrió dentro de la
> sección interna de los contenedores (`template`), Kubernetes crea un nuevo `ReplicaSet` bajo el capó para
> sustituir de manera transparente los `Pods` antiguos sin interrumpir el servicio.

La prueba de que la infraestructura mutó se hace evidente al listar los Pods individuales:
...
- `Validación de Hash`: Observamos el nuevo sufijo hash `75f9b59bc9` en el nombre de los Pods de usuarios,
  confirmando el cambio de plantilla.
- `Estado Exitoso`: Todos muestran la columna `READY` en `1/1`, lo que certifica que las nuevas instancias
  pasaron con éxito los exámenes de Actuator y ya se encuentran activas protegiendo y recibiendo tráfico en el clúster.

## 🧪 Probando el Comportamiento de los Probes en el Clúster

Con los Pods refrescados y Actuator corriendo, realizaremos las pruebas de consumo desde nuestra máquina de desarrollo
utilizando el puente de red hacia el clúster.

### 🔗 1. Obteniendo la URL pública del Servicio de Usuarios

Establecemos el túnel de comunicación local con Minikube para exponer nuestro recurso `svc-user-service`:

````bash
$ minikube service svc-user-service --url
http://127.0.0.1:61119
! Because you are using a Docker driver on windows, the terminal needs to be open to run it.
````

### 📡 2. Explorando el Catálogo Base de Actuator

Hacemos un `curl` directo a la raíz `/actuator` para validar el mapa de endpoints HTTP que expone el framework:

````bash
$ curl -v http://127.0.0.1:61119/actuator | jq
>
< HTTP/1.1 200
< Content-Type: application/vnd.spring-boot.actuator.v3+json
< Transfer-Encoding: chunked
< Date: Mon, 06 Jul 2026 17:42:12 GMT
<
{
  "_links": {
    "self": {
      "href": "http://127.0.0.1:61119/actuator",
      "templated": false
    },
    "beans": {
      "href": "http://127.0.0.1:61119/actuator/beans",
      "templated": false
    },
    "conditions": {
      "href": "http://127.0.0.1:61119/actuator/conditions",
      "templated": false
    },
    "configprops-prefix": {
      "href": "http://127.0.0.1:61119/actuator/configprops/{prefix}",
      "templated": true
    },
    "configprops": {
      "href": "http://127.0.0.1:61119/actuator/configprops",
      "templated": false
    },
    "env": {
      "href": "http://127.0.0.1:61119/actuator/env",
      "templated": false
    },
    "env-toMatch": {
      "href": "http://127.0.0.1:61119/actuator/env/{toMatch}",
      "templated": true
    },
    "info": {
      "href": "http://127.0.0.1:61119/actuator/info",
      "templated": false
    },
    "logfile": {
      "href": "http://127.0.0.1:61119/actuator/logfile",
      "templated": false
    },
    "loggers": {
      "href": "http://127.0.0.1:61119/actuator/loggers",
      "templated": false
    },
    "loggers-name": {
      "href": "http://127.0.0.1:61119/actuator/loggers/{name}",
      "templated": true
    },
    "threaddump": {
      "href": "http://127.0.0.1:61119/actuator/threaddump",
      "templated": false
    },
    "sbom": {
      "href": "http://127.0.0.1:61119/actuator/sbom",
      "templated": false
    },
    "sbom-id": {
      "href": "http://127.0.0.1:61119/actuator/sbom/{id}",
      "templated": true
    },
    "scheduledtasks": {
      "href": "http://127.0.0.1:61119/actuator/scheduledtasks",
      "templated": false
    },
    "mappings": {
      "href": "http://127.0.0.1:61119/actuator/mappings",
      "templated": false
    },
    "caches-cache": {
      "href": "http://127.0.0.1:61119/actuator/caches/{cache}",
      "templated": true
    },
    "caches": {
      "href": "http://127.0.0.1:61119/actuator/caches",
      "templated": false
    },
    "health-path": {
      "href": "http://127.0.0.1:61119/actuator/health/{*path}",
      "templated": true
    },
    "health": {
      "href": "http://127.0.0.1:61119/actuator/health",
      "templated": false
    },
    "metrics": {
      "href": "http://127.0.0.1:61119/actuator/metrics",
      "templated": false
    },
    "metrics-requiredMetricName": {
      "href": "http://127.0.0.1:61119/actuator/metrics/{requiredMetricName}",
      "templated": true
    },
    "refresh": {
      "href": "http://127.0.0.1:61119/actuator/refresh",
      "templated": false
    },
    "features": {
      "href": "http://127.0.0.1:61119/actuator/features",
      "templated": false
    }
  }
}
...
### 🟢 3. Verificando el Endpoint de Disponibilidad (`Readiness`)

Consultamos la sub-ruta específica que Kubernetes utiliza para determinar si el microservicio ya completó su
inicialización interna y puede atender peticiones de usuarios:

````bash
$ curl -v http://127.0.0.1:61119/actuator/health/readiness | jq
>
< HTTP/1.1 200
< Content-Type: application/vnd.spring-boot.actuator.v3+json
< Transfer-Encoding: chunked
< Date: Mon, 06 Jul 2026 17:45:34 GMT
<
{
  "status": "UP"
}
...
### 🔵 4. Verificando el Endpoint de Vitalidad (`Liveness`)

Interrogamos la ruta encargada de reportar que la máquina virtual de Java (JVM) se encuentra sana y respondiendo
estímulos correctamente:

````bash
$ curl -v http://127.0.0.1:61119/actuator/health/liveness | jq
>
< HTTP/1.1 200
< Content-Type: application/vnd.spring-boot.actuator.v3+json
< Transfer-Encoding: chunked
< Date: Mon, 06 Jul 2026 17:57:10 GMT
<
{
  "status": "UP"
}
...
### 📊 Conclusiones del Monitoreo Activo

La respuesta de estado `"status": "UP"` en ambos sub-recursos expuestos certifica que las políticas de autocuración del
clúster están sincronizadas con la lógica interna de Spring Boot.

| **Endpoint de Monitoreo**    | **Estado Obtenido** | **Comportamiento del Clúster en la Práctica**                                                                                                                                                                                                             |
|------------------------------|---------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `/actuator/health/readiness` | `UP`                | **Aceptando Tráfico:** El balanceador de carga del `Service` mantendrá al Pod dentro del grupo de enrutamiento activo. Si el estado cambiara a `DOWN`, el Pod se aislaría inmediatamente del tráfico, pero **no sería reiniciado**.                       |
| `/actuator/health/liveness`  | `UP`                | **Instancia Saludable:** El proceso principal de Linux está operativo. Si este endpoint respondiera un estado diferente o sufriera un *timeout*, el agente `Kubelet` finalizaría el contenedor y Kubernetes crearía automáticamente uno nuevo desde cero. |

## ⚙️ Control de Infraestructura: Configuración de Recursos en Pods (`Resources`)

Para garantizar la estabilidad, la justicia en el reparto de capacidades y evitar que un único contenedor monopolice
el clúster, Kubernetes nos permite definir de forma estricta los límites físicos de cómputo para nuestros contenedores.

En la especificación de un Pod se configuran dos conceptos clave:
...
### 📄 1. Actualización del Manifiesto `user-service-deployment.yml`

Inyectamos el bloque `resources` dentro de la especificación del contenedor de nuestro microservicio de usuarios:
    ...
    spec:
      containers:
        - image: magadiflo/user-service:latest
          ...
          # 🎛️ Asignación y control de hardware virtual
          resources:
            requests:
              memory: '512Mi'
              cpu: '400m'
            limits:
              memory: '1024Mi'
              cpu: '1000m'
# ... otras configuraciones (probes, variables de entorno)
````

### 📊 2. Diccionario de Recursos Asignados

| **Tipo**       | **Recurso** | **Valor**  | **Comportamiento en el Clúster**                                                                                                                                                      |
|----------------|-------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **`requests`** | `memory`    | **512Mi**  | Kubernetes **reserva** 512 MiB de memoria en el nodo. El Pod no será programado en un nodo que no disponga de esta cantidad de memoria libre.                                         |
| **`requests`** | `cpu`       | **400m**   | Kubernetes reserva **400 milicores** (equivalente al **40% de un CPU Core**). Esto garantiza una cantidad mínima de capacidad de procesamiento para los hilos de la JVM.              |
| **`limits`**   | `memory`    | **1024Mi** | Establece un **límite máximo de 1 GiB de memoria**. Si la aplicación supera este valor, el contenedor será finalizado por el kernel mediante un evento **OOMKilled (Out Of Memory)**. |
| **`limits`**   | `cpu`       | **1000m**  | Establece un **límite máximo de 1 CPU Core**. Si la aplicación intenta consumir más CPU, Kubernetes aplicará **throttling**, limitando su ejecución sin finalizar el contenedor.      |

### ⚠️ 3. Consecuencias Críticas de Superar los Límites

El comportamiento de Kubernetes ante un exceso de consumo varía drásticamente dependiendo del tipo de recurso:

````
Si el contenedor supera...
 ├───► Límite de CPU  ───► Throttling (Estrangulamiento) ───► El servicio se ralentiza, pero NO muere.
 └───► Límite de Memoria ──► OOMKilled (Out Of Memory)   ───► Kubernetes MATAL el contenedor de inmediato. 
````

- **Exceso de CPU (Mecanismo Elástico):** Si la aplicación necesita más de `1000m` de CPU, Kubernetes aplica
  `Throttling` (estrangulamiento). No destruye el proceso, simplemente le restringe los ciclos de reloj.
  La aplicación se volverá notablemente lenta, pero seguirá viva.
- **Exceso de Memoria (Mecanismo Rígido):** La memoria RAM no se puede estrangular. Si el microservicio supera
  los `1024Mi`, el kernel de Linux activa el protector del sistema y Kubernetes destruye el contenedor de forma
  fulminante bajo el estado `OOMKilled (Out Of Memory Killed)`, procediendo a reiniciarlo de inmediato.

### 📏 4. Unidades de Medida en Kubernetes: Decimal vs Binario

Para evitar imprecisiones de cálculo, Kubernetes utiliza el sistema estándar de prefijos binarios de la
`IEC (International Electrotechnical Commission)` en lugar del sistema decimal comercial.

- **Sufijos Decimales (Base 10):** `MB` (Megabyte), `GB` (Gigabyte).  
  1 MB = 1,000,000 bytes.
- **Sufijos Binarios (Base 2 - Preferidos por K8s):** `Mi` (Mebibyte), `Gi` (Gibibyte).  
  1 MiB = 1,048,576 bytes.

> 🔎 **Etimología de `Mi` (Mebibyte):**  
> Proviene de la contracción `Mega + binary + byte`. Aplica la misma regla para `Ki` (Kibibyte = 1,024 bytes) y
> `Gi` (Gibibyte = 1,024 MiB). Kubernetes exige estas unidades para asegurar una coincidencia exacta con la gestión de
> memoria real del hardware.

> 📐 **Regla de Oro de Producción**  
> 🚨 **El peligro de los contenedores sin límites:** Dejar un Deployment sin la sección resources se conoce como
> *"crear Pods con calidad de servicio BestEffort"*. Si el microservicio de usuarios sufre una fuga de memoria
> (memory leak), consumirá sin restricciones toda la RAM del nodo físico, desestabilizando al clúster por completo
> y provocando la caída en cascada de otros servicios vecinos. Configurar recursos siempre es obligatorio
> en producción.

## 🔄 Aplicación y Auditoría Física de Recursos en el Clúster

Con el archivo `user-service-deployment.yml` guardado, procederemos a impactar el clúster local para forzar la
redistribución de hardware virtual y comprobar la asignación mediante las herramientas de inspección de Kubernetes.

### 📝 Paso 1: Actualizar el Deployment de Usuarios

Enviamos los nuevos requerimientos de CPU y Memoria al clúster:
...
### ⏳ Paso 2: Monitorear la Transición y Rotación de Pods

Al consultar el estado de las instancias, observamos el proceso de reemplazo progresivo (`Rolling Update`). Kubernetes
detecta la nueva plantilla, crea un nuevo hash de `ReplicaSet` (`7c968cd49b`) y empieza a dar de baja los Pods
anteriores (hash `85d7cc7ddd`):

````bash
# 1. Inicio de la transición (Los Pods antiguos estaban en CrashLoopBackOff; entran los nuevos)
$ kubectl get pods
NAME                                READY   STATUS             RESTARTS         AGE
d-course-service-5b9bd8c648-dqkfh   1/1     Running            5 (7m15s ago)    3d22h
d-mysql-6989d9c76-pqh5m             1/1     Running            16 (7m15s ago)   11d
d-postgres-6f8f76ff47-vxpt9         1/1     Running            17 (7m15s ago)   11d
d-user-service-7c968cd49b-bpdth     0/1     Running            2 (8s ago)       2m30s
d-user-service-7c968cd49b-l4b5s     1/1     Running            3 (3m4s ago)     6m47s
d-user-service-85d7cc7ddd-8b85b     0/1     CrashLoopBackOff   6 (6s ago)       4h34m
d-user-service-85d7cc7ddd-sbs4v     0/1     CrashLoopBackOff   5 (107s ago)     4h34m
````

````bash
# 2. Descarga y aprovisionamiento de la tercera réplica nueva
$ kubectl get pods
NAME                                READY   STATUS              RESTARTS         AGE
d-course-service-5b9bd8c648-dqkfh   1/1     Running             5 (7m42s ago)    3d22h
d-mysql-6989d9c76-pqh5m             1/1     Running             16 (7m42s ago)   11d
d-postgres-6f8f76ff47-vxpt9         1/1     Running             17 (7m42s ago)   11d
d-user-service-7c968cd49b-bpdth     1/1     Running             2 (35s ago)      2m57s
d-user-service-7c968cd49b-l4b5s     1/1     Running             3 (3m31s ago)    7m14s
d-user-service-7c968cd49b-npbj8     0/1     ContainerCreating   0                2s
d-user-service-85d7cc7ddd-sbs4v     0/1     CrashLoopBackOff    5 (2m14s ago)    4h35m
````

````bash
# 3. Estado Final Estabilizado (Clúster Sano)
$ kubectl get pods
NAME                                READY   STATUS    RESTARTS         AGE
d-course-service-5b9bd8c648-dqkfh   1/1     Running   5 (8m23s ago)    3d22h
d-mysql-6989d9c76-pqh5m             1/1     Running   16 (8m23s ago)   11d
d-postgres-6f8f76ff47-vxpt9         1/1     Running   17 (8m23s ago)   11d
d-user-service-7c968cd49b-bpdth     1/1     Running   2 (76s ago)      3m38s
d-user-service-7c968cd49b-l4b5s     1/1     Running   3 (4m12s ago)    7m55s
d-user-service-7c968cd49b-npbj8     1/1     Running   0                43s
````

📊 **Análisis de Infraestructura:** El clúster purgó con éxito las instancias defectuosas e inicializó el nuevo trío de
Pods bajo el amparo de los límites establecidos. Notamos que `d-user-service-7c968cd49b-npbj8` arrancó de forma
limpia con `RESTARTS: 0`, demostrando que los probes de salud (`readiness/liveness`) le dieron el visto bueno.

### 🔍 Paso 3: Auditoría Interna del Manifiesto Activo

Para comprobar de forma empírica que Kubernetes inyectó los recursos en el motor de ejecución, ejecutamos una
inspección profunda al Deployment:

````bash
$ kubectl describe deployment d-user-service
Name:                   d-user-service
Namespace:              default
CreationTimestamp:      Tue, 30 Jun 2026 13:23:59 -0500
Labels:                 <none>
...
Pod Template:
  Labels:       app=d-user-service
  Annotations:  kubectl.kubernetes.io/restartedAt: 2026-07-04T20:15:12-05:00
  Containers:
   c-user-service:
    Image:      magadiflo/user-service:latest
    Port:       8001/TCP
    ...
    # 🎯 CONFIRMACIÓN DE RECURSOS ASIGNADOS:
    Limits:
      cpu:     1
      memory:  1Gi
    Requests:
    ...
    # 🛡️ Estado de las Políticas de Monitoreo:
    Liveness:   http-get http://:8001/actuator/health/liveness delay=15s timeout=5s period=10s #success=1 #failure=3
    Readiness:  http-get http://:8001/actuator/health/readiness delay=15s timeout=5s period=10s #success=1 #failure=3
    
    # 🌿 Variables de Entorno Activas desde ConfigMaps/Secrets:
    Environment:
      MY_POD_NAME:               (v1:metadata.name)
      MY_POD_IP:                 (v1:status.podIP)
      SPRING_PROFILES_ACTIVE:   <set to the key 'spring_profiles_active' of config map 'cm-user-service'>   Optional: false
      CONTAINER_PORT:           <set to the key 'container_port' of config map 'cm-user-service'>           Optional: false
      DB_HOST:                  <set to the key 'db_host' of config map 'cm-user-service'>                  Optional: false
      DB_PORT:                  <set to the key 'db_port' of config map 'cm-user-service'>                  Optional: false
      DB_NAME:                  <set to the key 'db_name' of config map 'cm-user-service'>                  Optional: false
...
OldReplicaSets:  d-user-service-58457b9ff9 (0/0 replicas created), d-user-service-646447fdbf (0/0 replicas created), d-user-service-874ff79fc (0/0 replicas created), d-user-service-64d7c75fd5 (0/0 replicas created), d-user-service-7449d45b76 (0/0 replicas created), d-user-service-5588cd6d77 (0/0 replicas created), d-user-service-8674595546 (0/0 replicas created), d-user-service-798588c6f5 (0/0 replicas created), d-user-service-59579bdb59 (0/0 replicas created), d-user-service-85d7cc7ddd (0/0 replicas created)
NewReplicaSet:   d-user-service-7c968cd49b (3/3 replicas created)
Events:
  Type    Reason             Age    From                   Message
  ----    ------             ----   ----                   -------
  Normal  ScalingReplicaSet  8m51s  deployment-controller  Scaled up replica set d-user-service-7c968cd49b from 0 to 1
  Normal  ScalingReplicaSet  4m35s  deployment-controller  Scaled down replica set d-user-service-85d7cc7ddd from 3 to 2
  Normal  ScalingReplicaSet  4m34s  deployment-controller  Scaled up replica set d-user-service-7c968cd49b from 1 to 2
  Normal  ScalingReplicaSet  99s    deployment-controller  Scaled down replica set d-user-service-85d7cc7ddd from 2 to 1
  Normal  ScalingReplicaSet  99s    deployment-controller  Scaled up replica set d-user-service-7c968cd49b from 2 to 3
  Normal  ScalingReplicaSet  65s    deployment-controller  Scaled down replica set d-user-service-85d7cc7ddd from 1 to 0
````

### 🎉 Conclusión de la Sección

El comando `describe` confirma el éxito absoluto de la operación:

1. **Límites Transparentes:** Kubernetes tradujo de forma automática nuestra configuración `1024Mi` a su equivalente
   directo binario comercial de 1Gi en la sección de límites.
2. **Garantía de Cómputo:** Los `400m` de CPU y los `512Mi` de memoria base quedaron reservados en los nodos para cada
   una de las 3 réplicas de forma independiente.
3. **Flujo de Eventos:** El historial de eventos (`Events`) evidencia cómo el orquestador escaló paulatinamente el
   nuevo `ReplicaSet` (`7c968cd49b`) de 0 a 3, mientras reducía a 0 el `ReplicaSet` antiguo, completando un despliegue
   de alta disponibilidad sin caídas de servicio.

## 🧪 Validación Funcional: Prueba de Carga y Consumo con Recursos Acotados

Establecer techos de memoria y CPU no sirve de nada si la aplicación se rompe internamente. Para certificar que la
JVM de Spring Boot levantó correctamente dentro de los nuevos límites físicos (`512Mi` base), realizaremos una
verificación de consumo de extremo a extremo.
...
### 📡 2. Petición HTTP de Diagnóstico al Endpoint `/info`

Disparamos un estímulo utilizando `curl` hacia el controlador de usuarios, procesando el flujo de bytes de
salida con `jq`:

````bash
$ curl -v http://127.0.0.1:65289/api/v1/users/info | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Mon, 06 Jul 2026 22:16:31 GMT
<
{
  "POD_NAME": "d-user-service-7c968cd49b-npbj8",
  "users": [
    {
      "id": 1,
      "name": "Martin",
...
### 🔍 3. Cuadro de Observaciones Técnicas

| **Atributo en el JSON** | **Valor Detectado**                            | **Interpretación y Estado de la Infraestructura**                                                                                                      |
|-------------------------|------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|
| **`POD_NAME`**          | `d-user-service-7c968cd49b-npbj8`              | ✅ **Enrutamiento Exitoso:** Respondió el Pod aprovisionado limpiamente con `RESTARTS: 0`, el mismo que fue identificado durante la auditoría anterior. |
| **`POD_IP`**            | `10.244.1.51`                                  | ✅ **Red Operativa:** Dirección IP asignada dinámicamente por el plugin de red (CNI) del clúster dentro del segmento de red de los nodos.               |
| **`config_text`**       | `"Configuraciones del perfil activo [dev]..."` | ✅ **Persistencia de Propiedades:** El contenedor continúa leyendo correctamente la configuración proveniente del ConfigMap externo.                    |
| **`users`**             | `[ { "id": 1, ... } ]`                         | ✅ **Conectividad a Base de Datos:** El pool de conexiones hacia la instancia de MySQL (`svc-mysql`) se estableció correctamente dentro del clúster.    |

### 🎉 Conclusión

El experimento confirma que el microservicio es perfectamente estable operando bajo restricciones físicas controladas.
Con una reserva base de 512Mi de memoria y un tope elástico de 1000m de CPU, la aplicación procesa peticiones de forma
fluida, asegurando un comportamiento predecible y blindando al clúster contra fugas de recursos descontroladas.

## 🧹 Simplificación del Manifiesto para Continuidad del Curso

Para agilizar el desarrollo y evitar bloqueos por restricciones físicas o tiempos de espera en las siguientes lecciones
teóricas/prácticas, removeremos temporalmente las capas de Monitoreo (`Probes`) y Cuotas de Hardware (`Resources`) del
microservicio de usuarios.

⚠️ **Nota de Arquitectura:** Esta limpieza se realiza únicamente con fines didácticos para facilitar la fluidez del
curso. Estas configuraciones representan el estándar obligatorio para entornos de `Staging` y `Producción`,
por lo que se reincorporarán en el tramo final del despliegue masivo.

### ✏️ 1. Modificación del Archivo `user-service-deployment.yml`

Para simplificar el manifiesto, realizaremos una "limpieza" eliminando por completo los bloques de control de hardware y
los sondeos de salud. A continuación, se detalla el contraste entre ambos estados del archivo:

#### 🛑 Configuración Original (Con Recursos y Probes)

Así se encontraba nuestro manifiesto con los bloques `resources`, `livenessProbe` y `readinessProbe`:
    ...
    spec:
      containers:
        - image: magadiflo/user-service:latest
          ...
          # 🚨 BLOQUE A ELIMINAR: Control de cuotas de hardware
          resources:
            requests:
              memory: '512Mi'
              cpu: '400m'
          ...
          # 🚨 BLOQUE A ELIMINAR: Sondeo de disponibilidad
          readinessProbe:
            httpGet:
              path: /actuator/health/readiness
              port: 8001
          ...
          # 🚨 BLOQUE A ELIMINAR: Sondeo de vitalidad
          livenessProbe:
            httpGet:
              path: /actuator/health/liveness
              port: 8001
...
# ... resto de variables de entorno hacia ConfigMaps y Secrets
````

#### 🧹 Configuración Simplificada (Versión de Laboratorio)

Luego de remover los bloques señalados, el archivo final queda optimizado y limpio para asegurar la continuidad del
curso:
    ...
    spec:
      containers:
        - image: magadiflo/user-service:latest
          ...
          ports:
            - containerPort: 8001
          # 💡 Capas avanzadas removidas temporalmente para agilizar el laboratorio
          env:
            - name: MY_POD_NAME
              valueFrom:
                fieldRef:
...
### 🚀 2. Aplicación de Cambios e Impacto en el Clúster

Inyectamos el plano de diseño simplificado en el API Server de Kubernetes:
...
Al remover estas propiedades de la sección del contenedor, Kubernetes se ve obligado a generar un nuevo ciclo de
rotación (`Rolling Update`) debido a la alteración de la plantilla base:

```` 
[ Eliminación de Probes/Resources ] ──► [ Cambio en Template Spec ] ──► [ Nuevo ReplicaSet ] ──► [ Rotación de Pods ]
````

Verificamos el estado de los componentes transcurridos unos segundos:

````bash
$ kubectl get pods
NAME                                READY   STATUS    RESTARTS       AGE
d-course-service-5b9bd8c648-dqkfh   1/1     Running   5 (50m ago)    3d23h
d-mysql-6989d9c76-pqh5m             1/1     Running   16 (50m ago)   11d
d-postgres-6f8f76ff47-vxpt9         1/1     Running   17 (50m ago)   11d
d-user-service-59579bdb59-8fn7p     1/1     Running   0              24s
d-user-service-59579bdb59-9xkp6     1/1     Running   0              27s
d-user-service-59579bdb59-d8lpg     1/1     Running   0              18s
````

- **Nuevo Hash de Identificación:** Los `Pods` pasaron al prefijo de su nuevo `ReplicaSet` `59579bdb59`.
- **Arranque Inmediato:** Al no existir un `initialDelaySeconds` de 15 segundos (removido junto con los probes),
  los Pods se marcan como `READY: 1/1` de forma casi instantánea en cuanto el proceso interno de Linux se ejecuta.
- **Calidad de Servicio (QoS):** Los Pods operan ahora bajo la categoría `BestEffort` (sin límites configurados), ideal
  para pruebas rápidas de conectividad local.
