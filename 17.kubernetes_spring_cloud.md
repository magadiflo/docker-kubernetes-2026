...
- `spring.config.import: 'kubernetes:'`. Habilita explícitamente la integración con Kubernetes como proveedor de
  ...
  > en el `pom.xml` y usando un archivo `bootstrap.yml`. Sin embargo, a partir de `Spring Boot 3.x` y
  > `Spring Boot 4.x`, el soporte para bootstrap viene apagado por defecto y es obligatorio declarar esta línea
