...
En esta lección verificaremos empíricamente que la anotación `@LoadBalanced` está funcionando correctamente — es decir,
que el balanceo ocurre **en memoria de Java** (`client-side`) a través de `Spring Cloud LoadBalancer`, en lugar del
`kube-proxy` (`server-side`) de Kubernetes.
...
### 📡 Realizando peticiones al endpoint `/info`

Ejecutamos tres peticiones consecutivas al endpoint de diagnóstico que expone los metadatos
del Pod gracias a la `Downward API`:

#### Petición 1 → Pod `-rbdjz`
...
| Petición | `POD_NAME`                        | `POD_IP`       |
|----------|-----------------------------------|----------------|
| 1        | `d-user-service-646447fdbf-rbdjz` | `10.244.0.219` |
| 2        | `d-user-service-646447fdbf-7vdm9` | `10.244.0.222` |
| 3        | `d-user-service-646447fdbf-b5p9g` | `10.244.0.225` |

Cada petición fue atendida por un Pod distinto — tanto el `POD_NAME` como el `POD_IP` cambian
en cada respuesta, confirmando que las tres réplicas participaron activamente en la distribución
del tráfico.
...
La evidencia es contundente: las tres peticiones consecutivas fueron atendidas por los tres Pods
distintos en orden secuencial (`-rbdjz` → `-7vdm9` → `-b5p9g`), lo que demuestra que
`Spring Cloud LoadBalancer` está aplicando un algoritmo **Round-Robin** para distribuir el
tráfico de forma equitativa entre las réplicas del microservicio `user-service`.

> 💡 Este comportamiento confirma el **balanceo client-side**: es el propio microservicio
> emisor quien elige a qué Pod enviar cada petición — consultando la caché local de IPs
> mantenida por `Spring Cloud Kubernetes` — sin depender del `kube-proxy` de Kubernetes.
