...
## 🧪 Probando la API REST de Cursos

Con el microservicio activo y conectado a `PostgreSQL local`, validamos los endpoints. Estas pruebas confirman que el
manejo de transacciones y el `versionado nativo` de `Spring Boot 4` funcionan perfectamente.

### 📑 Listar todos los cursos

Validamos que la lectura masiva desde PostgreSQL se mapea correctamente al record `CourseResponse`.
...
< Date: Fri, 13 Mar 2026 20:25:44 GMT
<
[
  {
    "id": 1,
...
Verificamos la recuperación selectiva de un recurso.

````bash
$ curl -v http://localhost:8002/api/v1/courses/3 | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 13 Mar 2026 20:26:16 GMT
<
{
  "id": 3,
  "name": "Kubernetes"
...
Probamos la persistencia y la generación de la cabecera Location.

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"name\": \"Spring WebFlux\"}" http://localhost:8002/api/v1/courses | jq
>
< HTTP/1.1 201
< Location: http://localhost:8002/api/v1/courses/4
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 13 Mar 2026 20:25:20 GMT
<
{
  "id": 4,
  "name": "Spring WebFlux"
...
Validamos que el `updateCourse` en la capa de servicio modifica correctamente el registro existente.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 13 Mar 2026 20:27:10 GMT
<
{
  "id": 1,
  "name": "Spring Boot"
...
Validamos la eliminación física del registro en PostgreSQL.

````bash
$ curl -v -X DELETE http://localhost:8002/api/v1/courses/4 | jq
>
< HTTP/1.1 204
< Date: Fri, 13 Mar 2026 20:27:49 GMT
<
````

## 🛡️ Pruebas de Robustez (Validación y Errores)

### ❌ Validación de campos (Bean Validation)

Enviamos un cuerpo vacío para disparar el `@NotBlank` del record `CourseRequest`.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 13 Mar 2026 20:28:19 GMT
< Connection: close
<
{
  "status": 400,
  ...
  "timestamp": "2026-03-13T15:28:19"
}
````

### 🔍 Recurso No Encontrado

Buscamos un ID inexistente para validar el `CourseNotFoundException`.

````bash
$ curl -v http://localhost:8002/api/v1/courses/99 | jq
>
< HTTP/1.1 404
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 13 Mar 2026 20:28:52 GMT
<
{
  "status": 404,
  "error": "Not Found",
  "message": "No se encuentra el curso con id [99]",
  "path": "/api/v1/courses/99",
  "timestamp": "2026-03-13T15:28:52"
}
````
