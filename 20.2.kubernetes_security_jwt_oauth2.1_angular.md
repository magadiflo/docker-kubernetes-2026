...
> 📝 Por ahora definimos una única ruta, la raíz (`''`), apuntando directamente al listado de cursos — es la
> pantalla de entrada de la aplicación. En fases posteriores iremos agregando rutas adicionales
> (`/courses/:id`, `/users/:id`, formularios de administración).

> 💡 **Lazy Loading Nativo:** Usamos `loadComponent` para importar dinámicamente la vista de cursos solo cuando el
> usuario navegue hacia ella, optimizando el tamaño del paquete inicial (bundle) de la aplicación.
