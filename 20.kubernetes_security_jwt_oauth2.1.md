...
Antes de implementar la propagación de tokens en la comunicación interna entre
`user-service` y `course-service`, necesitamos primero replantear nuestras reglas de autorización en el
`SecurityConfig` del `gateway-server`.

Actualmente, esas reglas fueron definidas de forma preliminar, sin considerar a fondo un caso importante: algunos
endpoints marcados como `permitAll()` en un microservicio terminan invocando, internamente, endpoints de otro
microservicio que sí requieren `authenticated()` o un rol específico. Esto puede generar fallos inesperados si no lo
resolvemos con cuidado antes de propagar el token entre servicios.
