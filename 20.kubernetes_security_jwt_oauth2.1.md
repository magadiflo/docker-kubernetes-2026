...
### 🛠️ Creación del Endpoint de Diagnóstico

Crearemos un controlador `@RestController` dentro del `gateway-server`. Para acceder a la información del cliente
autenticado utilizaremos la anotación `@RegisteredOAuth2AuthorizedClient`, la cual **permite inyectar directamente el
objeto `OAuth2AuthorizedClient`** administrado por Spring Security.

Este objeto encapsula toda la información obtenida durante el flujo `OAuth 2.1`, incluyendo:

- La configuración del cliente (`ClientRegistration`).
- El usuario autenticado (`Principal`).
- El `access_token`.
- El `refresh_token`.

El objetivo de este endpoint es únicamente verificar que el Gateway completó correctamente el flujo `Authorization Code`
y que Spring Security almacenó el cliente autorizado para futuras peticiones.

> 📌 **Nota de Diseño:**  
> Este controlador tiene un propósito exclusivamente didáctico y de diagnóstico. No forma parte de la aplicación final
> ni de la lógica de negocio. Una vez verificadas las pruebas podría eliminarse sin afectar el funcionamiento del
> sistema. En nuestro caso lo conservaremos únicamente como evidencia del proceso de construcción del proyecto.
...
> 💡 **Nota de Seguridad:**  
> No es necesario agregar este endpoint a la lista de rutas del `SecurityConfig`. La regla final
> `.anyExchange().authenticated()` protege automáticamente cualquier ruta que no haya sido configurada explícitamente
> como pública. En consecuencia, para acceder a este endpoint el usuario deberá completar previamente el proceso
> de autenticación mediante `OAuth 2.1`.
...
< Content-Type: application/json
< Content-Length: 1228
< Date: Sun, 19 Jul 2026 18:20:37 GMT
<
{
  "issuer": "http://localhost:9000",
  "authorization_endpoint": "http://localhost:9000/oauth2/authorize",
...
La correcta visualización y mapeo de los bloques `principalName`, `accessToken`, `refreshToken` y `clientRegistration`
en la salida nos otorga las siguientes certezas técnicas:

- 🛡️ El Gateway interceptó correctamente al usuario anónimo e inició el protocolo de manera automática.
- 🔑 El intercambio de `authorization_code` por tokens mediante el `token_endpoint` contra el servidor OAuth fue exitoso.
- 💾 La sesión reactiva persistió en el backend mediante el almacenamiento transparente del objeto
