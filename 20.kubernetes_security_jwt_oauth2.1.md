...
🎯 **Ajuste de Laboratorio:** La propiedad `.redirectUri(...)` apunta a `OAuth Debugger`, permitiéndonos interceptar el
`Authorization Code` de manera externa para realizar pruebas manuales mediante terminal. Adicionalmente, desactivamos el
consentimiento del usuario (`requireAuthorizationConsent(false)`) para agilizar el flujo del login.
