...
## 🛠️ Contenedores Utilitarios (MySQL/Postgres): Clientes de Línea de Comandos

En arquitecturas profesionales, no siempre disponemos de herramientas gráficas como `DBeaver`. En este apartado,
aprenderemos a desplegar contenedores utilitarios efímeros que actúan como clientes para auditar nuestras bases
de datos `c-mysql` y `c-postgres` directamente desde la red de `Docker`.

### ¿Qué es un Contenedor Utilitario?

Es una instancia temporal que no ejecuta un servicio persistente, sino que se utiliza para ejecutar una herramienta
específica (en este caso, el cliente `mysql` o `psql`) y se destruye inmediatamente después de cumplir su función.

### 🐬 Cliente CLI para MySQL

Utilizaremos la misma imagen de MySQL para lanzar una shell interactiva y conectarnos al servidor `c-mysql` que ya está
corriendo en nuestra red.

````bash
$ docker container run -it --rm --network docker-k8s-net mysql:8.0.41-debian /bin/sh
# mysql -h c-mysql -u admin -p
Enter password:
Welcome to the MySQL monitor.  Commands end with ; or \g.
...
- `-it`, habilita el modo interactivo y una TTY (terminal) para escribir comandos.
- `--rm`, cuando se detenga este contenedor, se eliminará automáticamente.
- `--network docker-k8s-net`, debe ser la misma red en la que se encuentra el contenedor `c-mysql`.
...
- `mysql -h c-mysql -u admin -p`, donde:
    ...
    - `-h c-mysql`, el host (`-h`) al que nos queremos conectar es el nombre del contenedor (`c-mysql`) donde está
      ejecutándose nuestra base de datos de mysql.
    - `-u admin`, el usuario (`-u`) es el que definimos al crear el contenedor de mysql, en nuestro caso es `admin`.
    - `-p`, el password es `magadiflo`, será ingresado en la línea siguiente.

### 🐘 Cliente CLI para PostgreSQL (Contenedor Utilitario)

Utilizaremos la misma imagen de `Postgres` para lanzar una shell interactiva y conectarnos al servidor `c-postgres`
que ya está corriendo en nuestra red.

````bash
$ docker container run -it --rm --network docker-k8s-net postgres:17-alpine /bin/sh
/ # psql -h c-postgres -U postgres
...
- `--rm`, cuando se detenga este contenedor, se eliminará automáticamente.
- `--network docker-k8s-net`, debe ser la misma red en la que se encuentra el contenedor `c-postgres`.
- `postgres:17-alpine`, imagen a partir de la cual crearemos un contenedor al vuelo, un utilitario.
- `/bin/sh` o `/bin/bash`, línea de comando que usaremos dentro del contenedor.

Al ingresar a la línea de comando vemos que escribí lo siguiente:

- `psql -h c-postgres -U postgres`, donde:
    ...
    - `-h c-postgres`, el host (`-h`) al que nos queremos conectar es el nombre del contenedor (`c-postgres`) donde está
      ejecutándose nuestra base de datos de postgresql.
    - `-U postgres`, el usuario (`-U`) es el que definimos al crear el contenedor de postgres, en nuestro caso es
      `postgres`.
    - Luego, cuando demos enter, en la siguiente línea pedirá ingresar el password que es `magadiflo`.

> **Detalle Técnico:**  
> A diferencia de `MySQL (donde se usa -u)`, en `PostgreSQL` la bandera para especificar el usuario es
> `-U (U mayúscula)`.

### 🏁 Conclusión: El valor de los Contenedores Efímeros

El uso de la bandera `--rm` en estos contenedores clientes es una de las mejores utilidades de Docker:

- `Sin Basura`: No dejas contenedores "muertos" en tu lista de docker ps `-a`.
- `Portabilidad`: No necesitas tener instalado `mysql-client` o `postgresql-client` en tu `Windows/Mac/Linux` local.
- `Seguridad`: La conexión se realiza dentro de la red virtual de Docker, sin necesidad de exponer puertos de base de
  datos al exterior si no es necesario.
