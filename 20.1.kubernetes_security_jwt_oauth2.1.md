...
## ✅ Prueba 4: Verificación de Invalidación de Sesión Central (Single Logout / SSO)

El objetivo de esta prueba es comprobar que el proceso de deslogueo no solo limpió el contexto del Gateway (BFF), sino
que también **invalidó la sesión en el Authorization Server, forzando la solicitud de credenciales al intentar ingresar
nuevamente.**

### 1️⃣ Pasos de la Prueba

1. Tras haber ejecutado el Logout en la `Prueba 3`, abrimos una pestaña y navegamos explícitamente a:
   `http://localhost:8090/auth/login`
2. **Resultado Observado:**  
   El Gateway redirige al navegador hacia la pantalla de autenticación del **Authorization Server**
   (`http://localhost:9000/login`) y este vuelve a solicitar las credenciales de usuario y contraseña.

### 2️⃣ Comportamiento Detallado de las Cookies durante el Flujo

- **Comportamiento diferido de JSESSIONID:**  
  Tras el logout, la sesión en la memoria del puerto 9000 es destruida. Sin embargo, el valor de la cookie `JSESSIONID`
  en el navegador no cambia de inmediato sino hasta que se realiza una nueva petición directa al puerto 9000
  (`/auth/login`). En ese instante, al detectar que la sesión anterior fue invalidada, Tomcat genera y responde con una
  nueva cookie `JSESSIONID` anónima para gestionar el formulario de login.

- **Confirmación de SLO (Single Logout):**  
  Si la sesión central no se hubiera invalidado, al ingresar a `/auth/login` el Authorization Server habría reutilizado
  la sesión previa sin pedir credenciales y nos habría redirigido a Angular de forma automática. Al solicitar login
  nuevamente, se confirma el éxito de la prueba.

> **💡 Conclusión de la Prueba:**
> La invalidación de sesión es efectiva en ambos niveles: el Gateway renovó su cookie `SESSION` inmediatamente durante
> el logout, y el Authorization Server invalidó la sesión SSO asociada a `JSESSIONID`, exigiendo reautenticación
> completa.

## ✅ Prueba 5: Verificación de Rutas Públicas (`GET /api/v1/courses`)

El objetivo de esta prueba es validar que las rutas configuradas explícitamente como públicas en el Gateway
(`.permitAll()`) sean accesibles libremente sin requerir una cookie de sesión activa ni tokens de autenticación.

### 1️⃣ Ejecución de la Petición Anónima

Ejecutamos un `curl` en modo verbose (o desde el navegador) hacia el endpoint de cursos expuesto por el Gateway sin
incluir ninguna cookie de sesión ni encabezado de autorización:
...
<
{
  "timestamp": "2026-07-29T22:22:08.036Z",
  "path": "/api/v1/courses",
  "status": 503,
  "error": "Service Unavailable",
  "requestId": "afe4f430-1"
}
````

### 3️⃣ Análisis del Resultado (Capa de Seguridad vs. Capa de Enrutamiento)

Para evaluar correctamente esta prueba es fundamental diferenciar entre el filtro de seguridad y el enrutador de red:

- **Paso por la Capa de Seguridad (Éxito):**  
  El Gateway no devolvió `401 Unauthorized` ni `403 Forbidden`, lo que confirma que la regla
  `.pathMatchers("/api/v1/courses/**").permitAll()` en `SecurityConfig` funcionó correctamente. La petición superó con
  éxito la barrera de autenticación anónima.
- **Fallo en la Capa de Enrutamiento Downstream (Esperado):**  
  Una vez superada la seguridad, el Gateway intentó resolver el destino mediante el balanceador de carga hacia el
  microservicio `courses-service`. Al estar este microservicio apagado en el entorno de pruebas, el Gateway respondió
  adecuadamente con un HTTP `503 Service Unavailable`.

> 💡 **Conclusión de la Prueba:**  
> La configuración de seguridad reactiva funciona correctamente para endpoints públicos. La respuesta `503` es la
> evidencia de que la petición atravesó de forma anónima la capa de autorización del Gateway y falló únicamente por
> la ausencia del microservicio destino.

## ✅ Prueba 6: Protección de Endpoints Privados

El objetivo de esta prueba es verificar que la capa de seguridad del Gateway intercepte las peticiones anónimas
dirigidas a recursos protegidos y active automáticamente el flujo de autenticación OIDC hacia el Authorization Server.

1. Asegurándonos de no tener una sesión activa (tras el logout), abrimos una pestaña en el navegador e ingresamos
   directamente a la URL de un recurso protegido: `http://localhost:8090/api/v1/courses/1`.
2. **Resultado Observado:**  
   El navegador es redirigido de forma transparente a través de una serie de códigos 302 Found hasta aterrizar en la
   pantalla de inicio de sesión central: `http://localhost:9000/login`.

> 💡 **Conclusión de la Prueba:**
> La protección de rutas privadas funciona de manera impecable. Las peticiones sin autenticación son bloqueadas en el
> borde de la arquitectura (Gateway) y conducidas de forma segura al proveedor de identidad central.

## ✅ Prueba 7: Comunicación entre Microservicios en Rutas Públicas (Sin Autenticación)

> 💡 **Nota de Configuración para Entorno Local:**  
> Para ejecutar y validar la comunicación directa entre microservicios (`user-service` y `course-service`) junto con
> el `gateway-server` y `authorization-server` en la máquina anfitriona (sin Kubernetes ni Service Discovery),
> se ajustaron temporalmente las propiedades de los archivos `application.yml` (desactivando kubernetes: import y
> enlaces lb://) y las clases `RestClientConfig.java` (removiendo `@LoadBalanced`).

Para ser exactos, todos estos archivos fueron los modificados para ejecutar nuestro proyecto en local:

- **business-domain/course-service/src/main/java/dev/magadiflo/course/app/config/RestClientConfig.java:**
    - Comenta la anotación `@LoadBalanced`.
- **business-domain/course-service/src/main/resources/application.yml:**
    - Apuntamos las credenciales a la base de datos local de Postgres:
        - `url`: jdbc:postgresql://localhost:5432/db_course_service
        - `username`: postgres
        - `password`: magadiflo
    - Apuntamos al microservicio de usuarios:
...
- **business-domain/user-service/src/main/resources/application.yml:**
    - Apuntamos las credenciales a la base de datos local de MySQL:
        - `url`: jdbc:mysql://localhost:3306/db_user_service
        - `username`: admin
        - `password`: magadiflo
    - Comentamos la configuración hacia kubernetes
    - Apuntamos al microservicio de cursos:
        - `base-url`: http://localhost:8002
- **infrastructure/gateway-server/src/main/resources/application.yml:**
    - Cambiamos temporalmente las uris de los microservicios:
        - `uri`: http://localhost:8002
        - `uri`: http://localhost:8001

### 1️⃣ Propósito de la Prueba

Verificar que:

1. El Gateway permita el tráfico anónimo hacia rutas configuradas como públicas (`/api/v1/courses`).
2. El microservicio de origen (`course-service`) pueda realizar llamadas HTTP síncronas sín autoconstruir tokens hacia
   `user-service` usando `RestClient` para enriquecer la respuesta (`loadRelations=true`).

### 2️⃣ Orden de Inicio de los Servicios en Local

Para asegurar la correcta inicialización y resolución de dependencias:

1. `authorization-server` (Puerto `9000`)
2. `user-service` (Puerto `8001`) y `course-service` (Puerto `8002`)
3. `gateway-server` (Puerto `8090`)

### 3️⃣ Petición de Prueba

Ejecutamos un `curl` solicitando el listado de cursos e indicando la bandera de carga de relaciones
`loadRelations=true`:

````bash
$ curl -v http://localhost:8090/api/v1/courses?loadRelations=true | jq
>
< HTTP/1.1 200 OK
...
< X-Frame-Options: DENY
< Content-Type: application/json
< Date: Thu, 30 Jul 2026 04:32:54 GMT
< Referrer-Policy: no-referrer
<
[
  {
...
### 5️⃣ Análisis del Flujo Técnico Interno

1. **Atraviesa el Gateway (Público):**  
   El Gateway reconoce la ruta `/api/v1/courses` como permitida (`.permitAll()`), la enruta hacia
   `http://localhost:8002` y no exige cabecera `Authorization`.
2. **Ejecución en `course-service`:**  
   El controlador recibe `loadRelations=true`. Recupera los cursos de la base de datos PostgreSQL local y detecta los
   IDs de usuarios asignados.
3. **Invocación Síncrona a `user-service`:**  
   Mediante el bean `userRestClient`, `course-service` efectúa una petición
   `GET http://localhost:8001/api/v1/users/....` Como es una petición anónima, el interceptor
   `bearerTokenPropagationInterceptor` no detecta un objeto `Jwt` en el `SecurityContextHolder` y envía la llamada HTTP
   estándar sin cabecera Bearer.
4. **Respuesta Consolidada:**  
   `user-service` responde con el detalle del usuario y `course-service` integra los datos en el JSON final entregado al
   cliente.

> 💡 **Conclusión de la Prueba:**  
> La comunicación inter-servicio síncrona mediante `RestClient` funciona correctamente. Los microservicios pueden
> comunicarse entre sí para complementar la información del dominio incluso en escenarios donde la petición original no
> requiere contexto de usuario autenticado.

## ✅ Prueba 8: Comunicación entre Microservicios con Propagación de Token OAuth2 (TokenRelay + Interceptor)

El objetivo de esta prueba es validar la propagación de seguridad en cadena (Token Propagation) a través de múltiples
capas de la arquitectura cuando se accede a un recurso protegido.

### 1️⃣ Flujo de Autenticación y Redirección

1. **Petición Inicial Protegida:**  
   Desde el navegador navegamos directamente al endpoint privado:
   `http://localhost:8090/api/v1/courses/2?loadRelations=true`
2. **Challenge de Autenticación:**  
   Al no poseer una cookie Ó activa, el Gateway bloquea la petición y redirige al usuario hacia el
   `Authorization Server` (`http://localhost:9000/login`).
3. **Autenticación del Usuario:**  
   Iniciamos sesión exitosamente con las credenciales del usuario `admin`.
4. **Redirección OAuth2 / OIDC:**  
   Tras completar el flujo de código de autorización, el Gateway registra la sesión reactiva del cliente y redirige al
   usuario hacia la aplicación Frontend (`http://localhost:4200/`).

### 2️⃣ Verificación de Acceso y Propagación

Con la sesión activa en el navegador, volvemos a realizar la solicitud al recurso protegido:
`http://localhost:8090/api/v1/courses/2?loadRelations=true`.

### 3️⃣ Respuesta Obtenida (`HTTP 200 OK`)
...
### 4️⃣ Análisis del Flujo de Propagación de Seguridad (Paso a Paso)
...
[ Navegador ] 
    ...
    ▼
[ Gateway Server (8090) ]
    │ (Filtro TokenRelay ➔ Inyecta Bearer JWT)
    ▼
[ Course Service (8002) ] ── (Valida JWT en Auth Server 9000)
    │ 
    │ (RestClientInterceptor ➔ Hereda Bearer JWT del SecurityContext)
    ▼
[ User Service (8001) ]   ── (Valida JWT en Auth Server 9000)
````

1. **Gateway (TokenRelay):** Recibe la cookie `SESSION`, recupera el `access_token` JWT guardado en el BFF e inyecta la
   cabecera `Authorization: Bearer <JWT>` en la petición enviada a `course-service`.
2. **course-service (Resource Server):** Spring Security intercepta la petición, verifica la firma del JWT enviando las
   claves a `http://localhost:9000` y establece el objeto `Authentication` en su `SecurityContextHolder`.
3. **Interceptor Personalizado (`RestClientConfig`):** Al procesar `loadRelations=true`, `course-service` debe consultar
   a `user-service`. El `ClientHttpRequestInterceptor` se ejecuta, extrae el objeto Jwt desde el
   `SecurityContextHolder` y le asigna el encabezado `Authorization: Bearer <jwt_token>` a la petición saliente del
   `RestClient`.
4. **user-service (Resource Server):** Recibe la petición HTTP interna, valida nuevamente la autenticidad del JWT
   recibido y entrega la información confidencial de las cuentas asociadas.

> 💡 **Conclusión Final:**  
> Se confirma el correcto funcionamiento del patrón `BFF + OAuth2 Resource Server + Token Propagation`.
> Las credenciales del usuario viajan de forma segura e inalterada a lo largo de toda la malla de microservicios,
> garantizando que ninguna llamada interna quede desprotegida o anónima.
