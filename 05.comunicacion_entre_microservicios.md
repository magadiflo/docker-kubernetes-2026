...
Tanto `RemoteUserNotFoundException` como `CommunicationException` son excepciones que hemos diseñado para capturar
la semántica del negocio. Nosotros decidimos cuándo se lanzan basándote en la lógica de nuestro `UserServiceClient`.
Son "manuales" porque tenemos el control total sobre su origen y su mensaje.
...
| `CommunicationException`      | **500 Internal Error**      | El `course-service` no pudo procesar la lógica debido a un dato inválido del remoto. |
