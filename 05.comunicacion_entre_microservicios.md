...
## 🛠️ Microservicio user-service: Consulta Masiva de Usuarios

Para soportar la hidratación de datos en el sistema distribuido, hemos dotado al `user-service` de una capacidad de
respuesta por lotes. Este endpoint permite que otros microservicios (como el `course-service`) recuperen múltiples
perfiles de usuario en una única transacción de red, mitigando el impacto de latencia.

### 📋 Responsabilidades de la Nueva Funcionalidad

- **Eficiencia de Red:** Reducción de llamadas múltiples al agrupar solicitudes de IDs.
- **Optimización de Persistencia:** Uso del método nativo de Spring Data para ejecuciones de alto rendimiento.
- **Interoperabilidad:** Exposición de un contrato claro mediante parámetros de consulta (RequestParams).

### 📂 Definición de Contrato: UserService

Se ha extendido la interfaz de servicio para incluir la búsqueda colectiva:
...
### 🏗️ Implementación de Lógica: UserServiceImpl

La implementación aprovecha la capacidad de `Spring Data JPA` para realizar búsquedas indexadas y un mapeo eficiente
hacia DTOs de respuesta.
...
### 📡 Exposición del Endpoint: UserController

El controlador expone esta funcionalidad mediante un parámetro de consulta, permitiendo flexibilidad en el número
de IDs enviados.
...
Se realizó una prueba de integración utilizando `curl` con el flag `-G` para el manejo correcto de parámetros de
consulta.

````bash
$ curl -v -G --data "userIds=1,3,5" http://localhost:8001/api/v1/users/by-ids | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 20:34:21 GMT
<
[
  {
    "id": 1,
...
## 🛰️ Microservicio course-service: Extensión del Cliente HTTP

Para habilitar la hidratación masiva de datos, hemos incorporado una nueva capacidad al `UserServiceClient`.
Este método actúa como el puente de comunicación hacia el nuevo endpoint de consulta por lotes del `user-service`,
permitiendo la recuperación eficiente de múltiples perfiles de usuario en una sola transacción.

### 📋 Responsabilidades de la Extensión

- **Construcción Dinámica de URIs:** Uso de `UriBuilder` para gestionar parámetros de consulta de forma segura.
- **Deserialización Genérica:** Implementación de tipos parametrizados para mantener la integridad de los datos.
- **Trazabilidad Distribuida:** Registro de logs para monitorear el flujo de datos entre microservicios.

### 🏗️ Implementación Técnica: UserServiceClient

El método `getUsersFromUserService(...)` utiliza el motor de `RestClient` de `Spring Boot 3.2+` para despachar la
solicitud de agregación.
...
#### 💡 ¿Qué es un `Parameterized Type`?

Un tipo parametrizado (`Parameterized Type`) es un `tipo genérico` al que se le han asignado uno o más argumentos
de tipo mediante la sintaxis `<...>`.
...
- `List` es un **tipo genérico.**
- `UserResponse` es el **argumento de tipo.**
- `List<UserResponse>` es un tipo parametrizado (`parameterized type`).

#### 💡 El Desafío de los Genéricos: ¿Por qué usar `ParameterizedTypeReference`?

Cuando consumimos endpoints que retornan colecciones genéricas como `List<UserResponse>`, nos enfrentamos a una
característica nativa de Java conocida como `Type Erasure` (Borrado de Tipos).

Durante la compilación, Java elimina la información de los tipos genéricos para mantener la compatibilidad con
versiones antiguas. Esto significa que, en tiempo de ejecución, la estructura `List<UserResponse>` pierde su identidad
y se convierte simplemente en una lista cruda (`List`).

#### ⚠️ El Problema: El sesgo de deserialización de Jackson

Si intentáramos mapear la respuesta utilizando clases tradicionales de la siguiente forma:

````bash
.body(List.class)
````

Spring Boot sabrá que debe construir una lista, pero **desconocerá por completo qué tipo de objetos van dentro.**
Como consecuencia, la librería de mapeo (Jackson) jugará a la segura y deserializará cada elemento como un mapa
genérico de Java (`LinkedHashMap`) en lugar de instancias reales de nuestro objeto `UserResponse`.
Esto provocará errores de casteo (`ClassCastException`) más adelante en el código.

#### 🛠️ La Solución: Conservación de tipos en ejecución

La clase `ParameterizedTypeReference` actúa como un contenedor de metadatos que logra capturar y conservar la
información completa del tipo genérico en tiempo de ejecución, burlando el `Type Erasure`.

Al pasar esta referencia al motor de `RestClient`:

````bash
.body(new ParameterizedTypeReference<>() {}); 
````

1. **Inferencia de Tipo (Target Typing):** Gracias al operador diamante (`<>`), el compilador deduce automáticamente
   que nos referimos a `List<UserResponse>` basándose en el tipo de la variable que recibe el resultado
   (`List<UserResponse> users`). Nos ahorra escribir código verboso.
2. **Garantía para Jackson:** Spring Boot recibe la instrucción exacta del tipo de destino y le ordena a Jackson que
   procese el JSON directamente en una lista de objetos `UserResponse` fuertemente tipados.

## 🛠️ Microservicio course-service: Extracción de Identidades

Como parte de la estrategia de Agregación de Datos, hemos incorporado un método de utilidad privado en
`CourseServiceImpl`. Su propósito es aislar la lógica de recolección de identificadores, permitiendo que el flujo
principal de hidratación sea más legible y enfocado en la orquestación.

### 🏗️ Implementación Técnica: CourseServiceImpl

El método utiliza el Stream API de Java para realizar una extracción eficiente y funcional de los metadatos
de relación.
...
## 🏗️ Hidratación Dinámica de Relaciones Distribuidas: Recuperar sus usuarios asociados

En arquitecturas de microservicios, la recuperación de datos relacionados requiere una orquestación eficiente para
evitar la sobrecarga del sistema. Hemos implementado un mecanismo de `Carga Bajo Demanda` que permite al
`course-service` enriquecer la información de los cursos consultando al `user-service` solo cuando es
estrictamente necesario.

### 📂 Definición de Contrato: CourseService

Se ha actualizado la interfaz para soportar el flag de control de relaciones:
...
### 🧠 Lógica de Orquestación: CourseServiceImpl

La implementación utiliza el método privado `loadRelations(...)` como el motor de unión entre ambos mundos
(Local y Remoto).
...
### 📡 Exposición en la Capa Web: CourseController

Se habilitan los parámetros opcionales en el controlador, estableciendo `false` como valor predeterminado
...
## 🧪 Validación de Resultados (Carga de Relaciones)

### 1️⃣ Consulta Simple (Rendimiento Óptimo)

Solicitud estándar sin parámetros adicionales. Retorna únicamente datos locales del curso.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 21:42:53 GMT
<
[
  {
    "id": 2,
...
### 2️⃣ Consulta Enriquecida (Hidratación Completa)

Activación del flag `loadRelations`. El servicio realiza el salto hacia el `user-service` para traer los detalles
de cada alumno.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 21:44:07 GMT
<
[
  {
    "id": 2,
...
## 🧹 Des-asignación Global de Usuarios (course-service)

En un ecosistema de microservicios, la eliminación de un recurso maestro (el Usuario) debe propagarse para mantener la
integridad de los datos. Hemos implementado un endpoint administrativo en el `CourseController` diseñado
específicamente para ser invocado durante el proceso de baja de un usuario.

### 🏗️ Implementación en el Controlador: CourseController

El endpoint delega la tarea al `CourseUserService`, el cual ejecuta una sentencia de eliminación directa por `userId`.
...
## 🧪 Validación de Consistencia (Prueba de Limpieza)

Para verificar la efectividad de la des-asignación global, realizamos un flujo de prueba completo.

### 1️⃣ Estado Inicial: Usuarios Inscritos

Consultamos los cursos con hidratación de datos para identificar al usuario `id=1` (Martin) inscrito en el
curso "Docker".
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 22:11:44 GMT
<
[
  {
    "id": 2,
...
### 2️⃣ Ejecución de Des-asignación

Invocamos el endpoint de borrado global para el `userId=1`.

````bash
$ curl -v -X DELETE http://localhost:8002/api/v1/courses/users/1 | jq
>
< HTTP/1.1 204
< Date: Wed, 18 Mar 2026 22:13:05 GMT
<
````

### 3️⃣ Verificación Final

Solicitamos nuevamente el listado enriquecido para confirmar que el usuario ha sido removido de todos los cursos.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 22:14:00 GMT
<
[
  {
    "id": 2,
...
En el curso `Docker (id: 2)`, el usuario "Martin" ya no aparece en la lista de users, quedando únicamente el resto de
inscritos. La limpieza se ha realizado con éxito sin afectar la entidad del curso.

## 🛰️ Microservicio user-service: Implementación de Cliente HTTP y Orquestación

Para asegurar la integridad de los datos en nuestro ecosistema, el `user-service` ahora asume la responsabilidad de
notificar al `course-service` cuando un usuario es removido del sistema maestro. Hemos implementado un cliente robusto
utilizando `RestClient`, con un manejo de excepciones diseñado para la resiliencia distribuida.

### ⚙️ Configuración del Cliente: RestClientConfig

Hemos externalizado la configuración para facilitar el despliegue en diferentes entornos (Desarrollo, QA, Producción).
...
### Propiedades de configuración (application.yml):
...
### 🏗️ Cliente de Infraestructura: CourseServiceClient

Este componente encapsula la lógica de red, exponiendo métodos semánticos que ocultan la complejidad de las peticiones
HTTP.
...
### 🛡️ Manejo de Excepciones del Cliente (GlobalExceptionHandler)

Dado que las peticiones externas pueden fallar por red o estado del servidor remoto, hemos blindado el servicio con
manejadores específicos:

| Excepción                     | Estado HTTP               | Propósito                                                                |
|-------------------------------|---------------------------|--------------------------------------------------------------------------|
| `RestClientResponseException` | `502 Bad Gateway`         | Captura errores específicos devueltos por el `course-service` (4xx/5xx). |
| `RestClientException`         | `503 Service Unavailable` | Maneja caídas totales del servicio, timeouts o errores de DNS.           |
...
### 🧠 Orquestación de Negocio: UserServiceImpl

La lógica de eliminación ahora es atómica y distribuida. Primero se asegura la existencia local, se elimina el registro
maestro y, finalmente, se limpia la relación en el servicio externo.
...
## 🧪 Validación Final: Eliminación Distribuida y Consistencia

En esta etapa final, validamos que la orquestación entre microservicios funciona correctamente. El objetivo es confirmar
que al eliminar un usuario en el `user-service`, el sistema propaga la `des-asignación` en el `course-service` de forma
transparente y exitosa.

### 1️⃣ Verificación de Relación Existente

Consultamos el curso de `Kubernetes (ID: 3)` para confirmar que el usuario `Melissa (ID: 2)`
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 22:56:31 GMT
<
{
  "id": 3,
  "name": "Kubernetes",
...
### 2️⃣ Ejecución de Baja en el Sistema Maestro (user-service)

Procedemos a eliminar al `usuario con ID: 2` desde su microservicio de origen. Esta operación dispara internamente la
llamada al `course-service` para la des-asignación global.

````bash
$ curl -v -X DELETE http://localhost:8001/api/v1/users/2 | jq
>
< HTTP/1.1 204
< Date: Wed, 18 Mar 2026 22:58:00 GMT
<
````

### 3️⃣ Verificación de Limpieza en course-service

Volvemos a consultar el curso de Kubernetes para validar que la lista de usuarios se ha actualizado automáticamente.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 22:59:06 GMT
<
{
  "id": 3,
  "name": "Kubernetes",
...
✅ Éxito: La colección de usuarios ahora está vacía. El `course-service` ha eliminado la relación física en la
base de datos local tras la notificación del `user-service`.

### 4️⃣ Confirmación de Eliminación Global

Finalmente, listamos todos los usuarios en el `user-service` para asegurar que el registro maestro ya no existe.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 18 Mar 2026 23:00:16 GMT
<
[
  {
    "id": 1,
...
Listado Final:  
El usuario con `ID: 2 (Melissa)` ha desaparecido del sistema, dejando únicamente los registros activos.
