...
- `d-mysql`: Es el nombre del `Deployment` que queremos exponer al resto del clúster.
- `--name=svc-mysql`: Define el nombre del recurso `Service`. **Dato importante:** Este nombre se convierte
  automáticamente en el `hostname` dentro de la red del clúster (otros Pods se conectarán a `svc-mysql:3306`).
- `--port=3306`: Puerto en el que el Service estará disponible dentro del clúster. Es la puerta de entrada para otros
  Pods.
- `--type=ClusterIP`: Asegura que la base de datos `solo sea accesible internamente`, protegiéndola de accesos externos
  al clúster.

> **💡 El comportamiento de `--port` y `--target-port`:**  
> Si no se especifica `--target-port` (en el comando `expose`), Kubernetes usa automáticamente el mismo valor definido
> en `--port`.
>
> - `--port`: Puerto en el que el Service `svc-mysql` estará disponible dentro del clúster.
> - `--target-port`: Puerto del contenedor donde realmente escucha la aplicación (MySQL).
>
> Por lo tanto:
>
> 1. **Si usas solo `--port=3306`:** Kubernetes mapea automáticamente `3306 (Service) → 3306 (contenedor)`.
     Para que funcione correctamente, este valor debe coincidir con el puerto en el que la aplicación
     realmente está escuchando (por ejemplo, MySQL en `3306`).
>
> 2. **Si defines ambos explícitamente:** Puedes usar un puerto de Service distinto
     (ej. `--port=80` `--target-port=3306`). En este caso, `--port` puede ser cualquier valor,
     pero `--target-port` debe coincidir con el puerto real donde la aplicación está escuchando.
...
- `--port=8001`: Puerto del `Service` que enruta el tráfico hacia los Pods. Como no se ha definido `--target-port`,
  Kubernetes usará este mismo valor como destino. Para que funcione correctamente, debe coincidir con el puerto en el
  que la aplicación realmente está escuchando (por ejemplo, `server.port` en Spring Boot).
...
## ⚖️ El Service como Balanceador de Carga (Load Balancer)

En Kubernetes, **un `Service` no es solo una IP estable; es un balanceador de carga interno.** Su función principal es
abstraer la complejidad de las IPs efímeras de los Pods y distribuir el tráfico entrante de manera equitativa entre
todas las réplicas disponibles mediante un algoritmo (generalmente `Round Robin`).

### 🔗 1. Obteniendo la URL de entrada

Para interactuar con el balanceador desde nuestro entorno local en Minikube, generamos el túnel de acceso:

````bash
$ minikube service svc-user-service --url
http://127.0.0.1:57280
! Because you are using a Docker driver on windows, the terminal needs to be open to run it.
````

### 📡 2. Conociendo el comando de escucha de logs (multicast)

El siguiente comando se queda escuchando todos los Pods que tengan el `Labels` `app=d-user-service`,
permitiéndonos identificar en tiempo real qué instancia procesa cada solicitud
(Otra manera de saber cuál es esa etiqueta es viendo el atributo `Selector` del `Deployment/Service`):

````bash
$ kubectl logs -l app=d-user-service -f --prefix
````

| Flag                    | Descripción                                                                                                                        |
|-------------------------|------------------------------------------------------------------------------------------------------------------------------------|
| `-l app=d-user-service` | Selecciona dinámicamente todos los Pods con ese label.                                                                             |
| `-f` **(Follow)**       | Mantiene la terminal abierta mostrando logs en tiempo real.                                                                        |
| `--prefix`              | Agrega el nombre del Pod al inicio de cada línea del log, permitiéndonos identificar qué instancia está procesando cada solicitud. |

> 💡 Este es el equivalente a hacer `tail -f` en varios servidores al mismo tiempo. Una herramienta
> muy útil en entornos con múltiples réplicas.

### 💻 3. Ejecutando el comando de escucha de logs (multicast)

Al ejecutar el comando de escucha, observamos cómo Kubernetes comienza a transmitir los logs de las tres réplicas
activas (`-qv99d`, `-nqcpr`, `-jzc9z`).

````bash
$ D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes-2026 (feature/section-14)
$ kubectl logs -l app=d-user-service -f --prefix
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]        drop index UK6dotkott2kjsp8vw4d0m25fb7
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T03:48:28.002Z DEBUG 1 --- [user-service] [           main] org.hibernate.SQL                        :
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]     alter table users
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]        add constraint UK6dotkott2kjsp8vw4d0m25fb7 unique (email)
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T03:48:28.129Z  INFO 1 --- [user-service] [           main] j.LocalContainerEntityManagerFactoryBean : Initialized JPA EntityManagerFactory for persistence unit 'default'
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T03:48:29.528Z  INFO 1 --- [user-service] [           main] o.s.d.j.r.query.QueryEnhancerFactories   : Hibernate is in classpath; If applicable, HQL parser will be used.
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T03:48:32.408Z  WARN 1 --- [user-service] [           main] JpaBaseConfiguration$JpaWebConfiguration : spring.jpa.open-in-view is enabled by default. Therefore, database queries may be performed during view rendering. Explicitly configure spring.jpa.open-in-view to disable this warning
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]    Pool: DataSourceConnectionProvider
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]    Pool: DataSourceConnectionProvider
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T03:48:40.403Z  INFO 1 --- [user-service] [           main] o.s.b.a.e.web.EndpointLinksResolver      : Exposing 1 endpoint beneath base path '/actuator'
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T03:48:41.615Z  INFO 1 --- [user-service] [           main] o.s.boot.tomcat.TomcatWebServer          : Tomcat started on port 8001 (http) with context path '/'
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T03:48:41.700Z  INFO 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Started UserServiceApplication in 78.816 seconds (process running for 80.622)
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]    Minimum pool size: undefined/unknown
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]    Maximum pool size: undefined/unknown
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T03:48:28.233Z  INFO 1 --- [user-service] [           main] org.hibernate.orm.core                   : HHH000489: No JTA platform available (set 'hibernate.transaction.jta.platform' to enable JTA platform integration)
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T03:48:28.508Z  INFO 1 --- [user-service] [           main] j.LocalContainerEntityManagerFactoryBean : Initialized JPA EntityManagerFactory for persistence unit 'default'
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T03:48:30.201Z  INFO 1 --- [user-service] [           main] o.s.d.j.r.query.QueryEnhancerFactories   : Hibernate is in classpath; If applicable, HQL parser will be used.
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]    Minimum pool size: undefined/unknown
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]    Maximum pool size: undefined/unknown
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T03:48:31.019Z  INFO 1 --- [user-service] [           main] org.hibernate.orm.core                   : HHH000489: No JTA platform available (set 'hibernate.transaction.jta.platform' to enable JTA platform integration)
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T03:48:31.408Z  INFO 1 --- [user-service] [           main] j.LocalContainerEntityManagerFactoryBean : Initialized JPA EntityManagerFactory for persistence unit 'default'
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T03:48:32.928Z  INFO 1 --- [user-service] [           main] o.s.d.j.r.query.QueryEnhancerFactories   : Hibernate is in classpath; If applicable, HQL parser will be used.
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T03:48:35.608Z  WARN 1 --- [user-service] [           main] JpaBaseConfiguration$JpaWebConfiguration : spring.jpa.open-in-view is enabled by default. Therefore, database queries may be performed during view rendering. Explicitly configure spring.jpa.open-in-view to disable this warning
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T03:48:42.407Z  INFO 1 --- [user-service] [           main] o.s.b.a.e.web.EndpointLinksResolver      : Exposing 1 endpoint beneath base path '/actuator'
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T03:48:42.922Z  INFO 1 --- [user-service] [           main] o.s.boot.tomcat.TomcatWebServer          : Tomcat started on port 8001 (http) with context path '/'
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T03:48:42.935Z  INFO 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Started UserServiceApplication in 68.995 seconds (process running for 78.605)
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T03:48:32.613Z  WARN 1 --- [user-service] [           main] JpaBaseConfiguration$JpaWebConfiguration : spring.jpa.open-in-view is enabled by default. Therefore, database queries may be performed during view rendering. Explicitly configure spring.jpa.open-in-view to disable this warning
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T03:48:39.601Z  INFO 1 --- [user-service] [           main] o.s.b.a.e.web.EndpointLinksResolver      : Exposing 1 endpoint beneath base path '/actuator'
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T03:48:41.320Z  INFO 1 --- [user-service] [           main] o.s.boot.tomcat.TomcatWebServer          : Tomcat started on port 8001 (http) with context path '/'
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T03:48:41.502Z  INFO 1 --- [user-service] [           main] d.m.user.app.UserServiceApplication      : Started UserServiceApplication in 72.927 seconds (process running for 78.892)
````

### 🚀 4. Generamos tráfico (el bombardeo de peticiones)

En otra terminal, vamos a lanzar 4 peticiones seguidas para que el balanceador tenga que trabajar.

````bash
$ curl -v http://127.0.0.1:57280/api/v1/users | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Sat, 02 May 2026 04:37:54 GMT
<
[]
````

### 🔍 5. Observando el balanceo en los logs

Volvemos a la terminal donde dejamos los logs corriendo y observamos el resultado:
...
...
...
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T04:37:54.323Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.a.c.c.C.[Tomcat].[localhost].[/]       : Initializing Spring DispatcherServlet 'dispatcherServlet'
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T04:37:54.327Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Initializing Servlet 'dispatcherServlet'
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T04:37:54.345Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Completed initialization in 18 ms
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T04:37:54.425Z  INFO 1 --- [user-service] [nio-8001-exec-1] d.m.user.app.controller.UserController   : Solicitud recibida para obtener todos los usuarios
[pod/d-user-service-6f74dd5668-qv99d/c-user-service] 2026-05-02T04:37:54.663Z DEBUG 1 --- [user-service] [nio-8001-exec-1] org.hibernate.SQL                        :
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]     select
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]         u1_0.id,
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]         u1_0.email,
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]         u1_0.name,
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]         u1_0.password
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]     from
[pod/d-user-service-6f74dd5668-qv99d/c-user-service]         users u1_0
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T04:38:08.065Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.a.c.c.C.[Tomcat].[localhost].[/]       : Initializing Spring DispatcherServlet 'dispatcherServlet'
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T04:38:08.069Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Initializing Servlet 'dispatcherServlet'
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T04:38:08.089Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Completed initialization in 19 ms
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T04:38:08.175Z  INFO 1 --- [user-service] [nio-8001-exec-1] d.m.user.app.controller.UserController   : Solicitud recibida para obtener todos los usuarios
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T04:38:08.373Z DEBUG 1 --- [user-service] [nio-8001-exec-1] org.hibernate.SQL                        :
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]     select
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         u1_0.id,
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         u1_0.email,
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         u1_0.name,
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         u1_0.password
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]     from
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         users u1_0
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T04:38:15.694Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.a.c.c.C.[Tomcat].[localhost].[/]       : Initializing Spring DispatcherServlet 'dispatcherServlet'
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T04:38:15.697Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Initializing Servlet 'dispatcherServlet'
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T04:38:15.715Z  INFO 1 --- [user-service] [nio-8001-exec-1] o.s.web.servlet.DispatcherServlet        : Completed initialization in 16 ms
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T04:38:15.793Z  INFO 1 --- [user-service] [nio-8001-exec-1] d.m.user.app.controller.UserController   : Solicitud recibida para obtener todos los usuarios
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service] 2026-05-02T04:38:16.030Z DEBUG 1 --- [user-service] [nio-8001-exec-1] org.hibernate.SQL                        :
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]     select
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]         u1_0.id,
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]         u1_0.email,
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]         u1_0.name,
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]         u1_0.password
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]     from
[pod/d-user-service-6f74dd5668-nqcpr/c-user-service]         users u1_0
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T04:39:10.347Z  INFO 1 --- [user-service] [nio-8001-exec-3] d.m.user.app.controller.UserController   : Solicitud recibida para obtener todos los usuarios
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service] 2026-05-02T04:39:10.351Z DEBUG 1 --- [user-service] [nio-8001-exec-3] org.hibernate.SQL                        :
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]     select
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         u1_0.id,
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         u1_0.email,
[pod/d-user-service-6f74dd5668-jzc9z/c-user-service]         u1_0.name,
...
| Observación                          | Detalle                                                                                                                                                                                  |
|--------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 🔄 **Balanceo distribuido**          | Las 4 peticiones fueron atendidas por los 3 Pods distintos: `-qv99d`, `-jzc9z`, `-nqcpr` y `-jzc9z`. ¡Los tres Pods estuvieron en acción!                                                |
| 🥶 **Cold Start (arranque en frío)** | Las primeras 3 peticiones muestran `Initializing Spring DispatcherServlet` porque los Pods nunca habían recibido tráfico. Spring inicializó el `DispatcherServlet` justo en ese momento. |
| 🔥 **Contenedor "caliente"**         | En la 4ª petición al Pod `-jzc9z`, ya no aparece la inicialización — el contenedor estaba listo y respondió directamente.                                                                |

> 💡 **Sobre el orden del balanceo:** No esperes siempre un ciclo perfecto `1 → 2 → 3`. Kubernetes
> puede repetir un Pod antes de pasar al siguiente dependiendo de la carga o de cómo el `Service`
> gestiona las conexiones activas. Con 10 o más peticiones verás movimiento entre los tres
> Pods sin duda.

### 📌 Conclusión General

Hemos comprobado **empíricamente** que el `Service` es la pieza clave para la escalabilidad en
Kubernetes. Actúa como una fachada única que garantiza:

| Propiedad                     | Descripción                                                                                                                |
|-------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| 🟢 **Alta Disponibilidad**    | Si un Pod fallara, el `Service` redirige el tráfico automáticamente a los restantes.                                       |
| 👁️ **Transparencia**         | El cliente nunca sabe qué IP interna de Pod está respondiendo; solo interactúa con el balanceador.                         |
| 📈 **Escalabilidad Elástica** | Podemos pasar de 3 a 100 réplicas y el comando de acceso seguiría siendo el mismo — el balanceo se ajusta automáticamente. |

## 🛡️ Verificando el Comportamiento de los Pods Escalados ante un Fallo

En un entorno de microservicios, la resiliencia es vital. Hasta este punto, contamos con `3 réplicas` del microservicio
`user-service` distribuidas en el clúster. Debido a la naturaleza dinámica de Kubernetes y a las pruebas de escalado
que estuve realizando antes de hacer esta documentación, los nombres de los Pods pueden variar, pero el comportamiento
esperado de autocuración se mantiene constante.

Actualmente, nuestro clúster presenta el siguiente estado operativo:

```bash
$ kubectl get pods
NAME                              READY   STATUS    RESTARTS        AGE
d-mysql-6997b8d9fc-bkcr5          1/1     Running   12 (46m ago)    12d
d-user-service-6f74dd5668-gh5rf   1/1     Running   2 (2m35s ago)   23m
d-user-service-6f74dd5668-nb88s   1/1     Running   1 (37s ago)     23m
d-user-service-6f74dd5668-nqcpr   1/1     Running   6 (10m ago)     2d22h
```

A continuación, ejecutaremos una falla simulada en una de estas instancias para observar cómo Kubernetes detecta la
anomalía y restaura el estado deseado de forma automática.
...
Ejecutamos una petición al endpoint `/simulate-error`, diseñado para provocar un fallo controlado
cerrando el contexto de Spring:

````bash
$ curl -v http://127.0.0.1:57420/api/v1/users/simulate-error | jq
>
* Request completely sent off
  0      0   0      0   0      0      0      0           00:27              0* Empty reply from server
  0      0   0      0   0      0      0      0           00:27              0
* shutting down connection #0
curl: (52) Empty reply from server
````

El error `Empty reply from server` confirma que el servidor cerró la conexión antes de responder —
el contenedor se apagó abruptamente.

### 🔍 Estado de los Pods justo después del fallo

Si consultamos los Pods inmediatamente tras el error, podemos ver a Kubernetes reaccionando en
tiempo real:

````bash
$ kubectl get pods
NAME                              READY   STATUS             RESTARTS        AGE
d-mysql-6997b8d9fc-bkcr5          1/1     Running            12 (47m ago)    12d
d-user-service-6f74dd5668-gh5rf   1/1     Running            2 (3m38s ago)   24m
d-user-service-6f74dd5668-nb88s   0/1     CrashLoopBackOff   1 (10s ago)     24m
d-user-service-6f74dd5668-nqcpr   1/1     Running            6 (11m ago)     2d22h
````

> 💡 El Pod `-nb88s` aparece en estado `CrashLoopBackOff` con `READY: 0/1`, lo que significa que
> su contenedor falló y Kubernetes está esperando para reiniciarlo. Los otros dos Pods siguen
> activos y disponibles.

### ⚙️ Kubernetes maneja la falla automáticamente

Según la política de reinicio `Always` configurada por defecto en los `Deployments`, Kubernetes
reinicia automáticamente el contenedor fallido. Durante ese proceso, las otras dos réplicas
continúan atendiendo peticiones sin interrupción.

Podemos comprobarlo accediendo a un endpoint válido inmediatamente después del error:

````bash
$ curl -v http://127.0.0.1:57420/api/v1/users | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Mon, 04 May 2026 21:22:33 GMT
<
[]
````

✅ `HTTP 200 OK` — la aplicación sigue funcionando porque el `Service` redirigió la petición a uno
de los Pods activos. **No hubo interrupción visible para el cliente.**

### 📊 Estado de los Pods tras la recuperación

Una vez que Kubernetes reinicia el contenedor, todos los Pods vuelven a estar en estado `Running`:

```bash
$ kubectl get pods
NAME                              READY   STATUS    RESTARTS        AGE
d-mysql-6997b8d9fc-bkcr5          1/1     Running   12 (50m ago)    12d
d-user-service-6f74dd5668-gh5rf   1/1     Running   2 (6m57s ago)   27m
d-user-service-6f74dd5668-nb88s   1/1     Running   2 (3m29s ago)   27m
d-user-service-6f74dd5668-nqcpr   1/1     Running   6 (14m ago)     2d22h
````

El Pod `-nb88s` pasó de `RESTARTS: 1` a `RESTARTS: 2`, confirmando que fue el que ejecutó el
endpoint de error y se recuperó automáticamente.

### 🏆 Alta Disponibilidad y Resiliencia

Este experimento demuestra en la práctica una de las fortalezas clave de Kubernetes:

| Propiedad                  | Comportamiento observado                                                            |
|----------------------------|-------------------------------------------------------------------------------------|
| 🟢 **Alta Disponibilidad** | Los otros 2 Pods siguieron atendiendo peticiones mientras el tercero se recuperaba. |
| 🔄 **Auto-recuperación**   | Kubernetes detectó el fallo y reinició el contenedor sin intervención manual.       |
| 👁️ **Transparencia**      | El cliente no percibió ninguna interrupción gracias al balanceo del `Service`.      |

### 📉 Reduciendo las réplicas

Una vez verificado el comportamiento, reducimos el `Deployment` a una sola réplica:
...
$ kubectl get pods
NAME                              READY   STATUS    RESTARTS       AGE
d-mysql-6997b8d9fc-bkcr5          1/1     Running   12 (54m ago)   12d
d-user-service-6f74dd5668-nqcpr   1/1     Running   6 (17m ago)    2d22h
```

✅ Kubernetes eliminó automáticamente los Pods sobrantes, dejando únicamente una instancia activa
del microservicio.

## 📜 Pasando del Método Imperativo al Declarativo: El Poder de los archivos YAML

Hasta ahora, hemos gestionado nuestro clúster de manera `imperativa`, ejecutando comandos directos en la terminal
para obtener resultados inmediatos. Si bien esto es útil para pruebas rápidas, en entornos productivos se utiliza
el `método declarativo`, donde definimos el "estado deseado" en archivos de configuración para que Kubernetes
se encargue de mantenerlo.

> 💡 **Imperativo vs Declarativo:**
> - **Imperativo:** Le dices a Kubernetes *cómo* hacer algo paso a paso (`kubectl expose`, `kubectl create`...).
> - **Declarativo:** Le dices a Kubernetes *qué* quieres que exista y él se encarga del resto (archivos `YAML`).

### 1. El punto de partida: Gestión Imperativa

Como recordatorio, estos fueron los comandos `kubectl expose` que utilizamos para comunicar nuestras bases de datos
y microservicios:

````bash
# Creación del servicio para la base de datos (Interno)
$ kubectl expose deployment d-mysql --port=3306 --type=ClusterIP --name=svc-mysql

# Creación del servicio para el microservicio (Externo/Balanceado)
$ kubectl expose deployment d-user-service --port=8001 --type=LoadBalancer --name=svc-user-service
````

### 2. Estado actual de los servicios

Verificamos los servicios que tenemos creados hasta este momento:

````bash
$ kubectl get services
NAME               TYPE           CLUSTER-IP     EXTERNAL-IP   PORT(S)          AGE
kubernetes         ClusterIP      10.96.0.1      <none>        443/TCP          17d
svc-mysql          ClusterIP      10.100.3.217   <none>        3306/TCP         11d
svc-user-service   LoadBalancer   10.98.119.92   <pending>     8001:31169/TCP   6d
````

A partir de estos dos servicios existentes (`svc-mysql` y `svc-user-service`) generaremos sus correspondientes
archivos `YAML` declarativos con los que trabajaremos a partir de ahora.

### 3. Hacia la Infraestructura como Código (IaC)

En este apartado, daremos el salto hacia la Infraestructura como Código. En lugar de escribir comandos,
utilizaremos los servicios ya creados (`svc-mysql` y `svc-user-service`) como base para generar archivos `.yml`.

Esto nos permitirá:

- Versionar nuestras configuraciones en Git.
- Documentar de forma exacta cómo debe comportarse el servicio.
- Replicar el entorno en cualquier otro clúster de forma idéntica.

> 💡 **Próximo Paso:** Utilizaremos una técnica de "ingeniería inversa" con `kubectl` para extraer la definición
> actual de estos servicios en formato YAML y limpiarla para que sea reutilizable.

### 4. Generando el archivo YAML para el servicio `svc-mysql`

Una de las formas más eficientes de migrar al modelo declarativo es realizar ingeniería inversa sobre los recursos que
ya están funcionando en el clúster. Para ello, utilizaremos la capacidad de `kubectl` de exportar definiciones
en formato `YAML`.

#### A. Inspeccionando la configuración actual en formato YAML

Antes de generar el archivo, veamos cómo Kubernetes representa internamente el servicio `svc-mysql` en formato `YAML`:

````bash
$ kubectl get service svc-mysql -o yaml
apiVersion: v1
kind: Service
metadata:
  creationTimestamp: "2026-04-22T22:53:55Z"
  name: svc-mysql
  namespace: default
  resourceVersion: "27035"
  uid: 7cc904d8-6e58-408a-b6a1-d715cae37601
spec:
  clusterIP: 10.100.3.217
  clusterIPs:
  - 10.100.3.217
...
> 💡 El flag `-o yaml` le indica a `kubectl` que devuelva la configuración del recurso en formato
> `YAML` en lugar del formato tabular por defecto. Esto nos permite exportar la configuración
> exacta que Kubernetes tiene registrada para ese recurso.

#### B. Exportando la configuración al archivo `mysql-service.yml`

Aprovechamos ese mismo comando para redirigir la salida directamente a un archivo `YAML`.
Posicionados en la raíz del proyecto, ejecutamos:

````bash
D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes-2026 (feature/section-14)
$ kubectl get service svc-mysql -o yaml > .\kubernetes\services\mysql-service.yml
````

> 📌 **Nota:** La ruta `.\kubernetes\services\` debe crearse previamente, ya que es el directorio
> donde almacenaremos todos los archivos de configuración de servicios.

#### C. Verificando el archivo generado

Al inspeccionar el archivo `mysql-service.yml` creado, observamos que contiene la configuración
...
### 5. Limpiando el archivo YAML del servicio `svc-mysql`

El archivo generado en el apartado anterior contiene metadata interna de Kubernetes (como `uid`,
`resourceVersion` y `creationTimestamp`) que no es necesaria cuando gestionamos la configuración
de forma declarativa. A continuación realizamos una limpieza dejando únicamente los campos
esenciales:
...
- `port`: Puerto en el que el servicio `svc-mysql` estará disponible dentro del clúster.
  Cualquier Pod que quiera conectarse a MySQL deberá hacerlo a través de este puerto (`3306`).

- `targetPort`: **Puerto del contenedor donde realmente escucha la aplicación `(MySQL)`**.
  Este valor debe coincidir con el puerto en el que la aplicación está escuchando,
  ya que es el destino real del tráfico (Ej. en una aplicación de Spring Boot, ese puerto es el definido en el
  `server.port`).
    - El servicio redirige el tráfico desde `port` hacia `targetPort` a nivel de red. Ambos pueden ser **distintos** —
      Kubernetes traduce automáticamente las peticiones, de forma similar al mapeo de puertos en Docker
      (`puerto_host:puerto_contenedor`), aunque en este caso ocurre dentro del clúster.
    - En este ejemplo ambos son `3306` por conveniencia, pero no es obligatorio.

- `protocol`: Protocolo usado en la comunicación. Por defecto es `TCP`, el más común en
  servicios como bases de datos.

- `selector`: Define a qué Pods debe redirigir el tráfico este servicio. El selector
  `app: d-mysql` busca todos los Pods que tengan esa etiqueta, que están definidos en la sección
  `spec.template.metadata.labels` del archivo `mysql-deployment.yml`. Esto garantiza que
  `svc-mysql` conecta correctamente con los Pods gestionados por ese `Deployment`.

- `type: ClusterIP`: Expone el servicio únicamente dentro del clúster. Otros Pods pueden
  acceder a `svc-mysql`, pero no es accesible desde fuera (navegador, curl, Postman, etc.).

> ### 🎯 ¿`port` y `targetPort` pueden ser distintos?
>
> ¡Sí, absolutamente! Y la analogía con Docker es perfecta para entender la redirección:
>
> ````
> Docker:  -p 8080:3306   →  HOST_PORT:CONTAINER_PORT
> K8s:     port: 8080        targetPort: 3306
>          (puerto Service)  (puerto contenedor) 
> ````
>
> Por ejemplo, si configuramos una base de datos MySQL, esto es completamente válido:
>
> ````yml
> ports:
>   - port: 9000        # Puerto "público" dentro del cluster (donde otros pods llaman)
>     targetPort: 3306  # Puerto real donde el contenedor de MySQL está escuchando
> ````
>
> **Kubernetes redirigirá automáticamente cualquier petición que llegue al servicio por el puerto `9000`
> hacia el puerto `3306` del contenedor dentro del Pod.** Este proceso ocurre a nivel de red dentro del clúster.
>
> En muchos casos (como en nuestro microservicio de Spring Boot) usamos el mismo número en ambos
> para simplificar, pero **no es un requisito técnico.**

### 6. Generando y limpiando el archivo YAML para el servicio `svc-user-service`

Repetimos el proceso de "Ingeniería Inversa" para nuestro microservicio principal, extrayendo su configuración actual a
un archivo físico.

````bash
D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes-2026 (feature/section-14)
$ kubectl get service svc-user-service -o yaml > .\kubernetes\services\user-service-service.yml
````

Siguiendo la misma lógica de simplificación, editamos `user-service-service.yml` para dejar la estructura esencial.
En este caso, el tipo de servicio es `LoadBalancer`, ya que este microservicio sí debe ser accesible desde el exterior.
...
|------------|-----------------------|---------------------------------------------------------------------------|
| `name`     | `svc-user-service`    | Nombre DNS interno para el microservicio.                                 |
| `type`     | `LoadBalancer`        | Expone el servicio externamente y balancea el tráfico entre las réplicas. |
| `selector` | `app: d-user-service` | Vincula este servicio con los Pods del Deployment de usuarios.            |

## 📋 Gestión Declarativa con el `user-service-deployment.yml`: Eliminando Recursos con YAML

Una de las mayores ventajas del método declarativo es que el archivo YAML se convierte en el "ID" o la referencia del
recurso. Ya no necesitamos recordar nombres exactos en la terminal; si tenemos el archivo, tenemos el control sobre el
recurso.

### ¿Qué vamos a hacer?

En este apartado trabajaremos exclusivamente con los archivos `YAML` que hemos creado anteriormente,
para ver lo sencillo y limpio que es gestionar recursos de Kubernetes de forma declarativa.

### 🔍 1. Verificando el estado actual

Antes de proceder, verificamos los `Deployments` activos. Actualmente, nuestro microservicio de usuarios
está corriendo de forma normal:

````bash
$ kubectl get deployments
NAME             READY   UP-TO-DATE   AVAILABLE   AGE
d-mysql          1/1     1            1           14d
d-user-service   1/1     1            1           11d
````

### 🗑️ 2. Eliminando el Deployment `d-user-service`

> 💡 **Imperativo vs Declarativo:** Hasta ahora, si queríamos eliminar un recurso de forma `imperativa`,
> debíamos especificar su tipo y su nombre exacto:
> ```bash
> # Método imperativo (basado en nombres)
> $ kubectl delete deployment d-user-service
> ```

Sin embargo, en el método `declarativo`, simplemente le indicamos a Kubernetes que queremos eliminar lo que sea
que esté definido dentro de nuestro archivo de configuración `YAML`:

````bash
# Método declarativo (basado en archivos)
D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes-2026 (feature/section-14)
...
> 📌 **Importante:** Este comando **no elimina el archivo** `user-service-deployment.yml`. Lo que
> elimina es el recurso `Deployment` (`d-user-service`) que está definido dentro de ese archivo.
> El archivo de configuración permanece intacto.

### ✅ 3. Verificando la eliminación

Tras ejecutar la eliminación `declarativa`, Kubernetes se encarga de terminar todos los procesos asociados a ese
manifiesto. Al listar los recursos, observamos que tanto el `Deployment` como sus `Pods` hijos han desaparecido.

Listamos nuevamente los `Deployments` para confirmar que `d-user-service` fue eliminado:
...
Y si listamos los Pods, confirmamos que solo queda el Pod de MySQL:

````bash
$ kubectl get pods
NAME                       READY   STATUS    RESTARTS       AGE
d-mysql-6997b8d9fc-bkcr5   1/1     Running   14 (11m ago)   14d
````

✅ El `Deployment` `d-user-service` y sus Pods asociados fueron eliminados correctamente, sin afectar al `Deployment`
de MySQL ni a su Pod.

## 📋 Gestión Declarativa con el `user-service-deployment.yml`: Creando Recursos con YAML

Una vez que tenemos nuestros archivos YAML limpios y estructurados, pasamos a la fase de despliegue.
A diferencia de los comandos de creación `(create)`, el comando `apply` es inteligente: lee el archivo,
observa el estado actual del clúster y realiza solo los cambios necesarios para alcanzar el `"estado deseado"`.

### ¿Qué vamos a hacer?

Ahora que eliminamos el `Deployment` `d-user-service` en el apartado anterior, lo recrearemos
usando el mismo archivo `YAML` — demostrando que con un solo comando podemos restaurar toda la
configuración del recurso.

### 🚀 1. Creando el Deployment

Aplicamos el archivo de configuración para recrear el `Deployment` utilizando el flag `-f (file)` para indicarle a
Kubernetes que tome la configuración definida en nuestro archivo YAML:
...
> **🧠 ¿Por qué usar `apply` en lugar de `create`?**  
> En tu flujo de trabajo profesional, notarás que `apply` es el estándar por una razón técnica llamada `Idempotencia`:
>
> - `create`: Si intentas ejecutarlo y el recurso ya existe, el comando fallará con un error.
> - `apply`: Si el recurso ya existe, Kubernetes comparará tu archivo YAML con el recurso vivo.
    Si hay cambios (ej. subiste las réplicas de 1 a 3), los aplicará. Si no hay cambios, no hará nada.
>
> 💡 **Conclusión:** Con los archivos YAML, tu infraestructura se vuelve auditable y reproducible. Cualquier miembro del
> equipo puede tomar tu carpeta `/kubernetes` y replicar exactamente el mismo entorno en cuestión de segundos.

### ✅ 2. Verificando los recursos creados

Tras la ejecución, Kubernetes orquesta la creación de los objetos necesarios. Podemos confirmar que el `Deployment`
y su correspondiente `Pod` han sido levantados exitosamente.
...
$ kubectl get pods
NAME                              READY   STATUS    RESTARTS       AGE
d-mysql-6997b8d9fc-bkcr5          1/1     Running   14 (35m ago)   14d
d-user-service-59ffdb9645-9qfl7   1/1     Running   0              113s
````

✅ El nuevo Pod `d-user-service-59ffdb9645-9qfl7` tiene `RESTARTS: 0` y `AGE: 113s`, confirmando que fue recién creado
a partir del archivo `YAML`.

### 🧪 3. Probando el microservicio

Generamos la URL del servicio y verificamos que el microservicio responde correctamente:
...
````bash
$ curl -v http://127.0.0.1:56483/api/v1/users | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Wed, 06 May 2026 21:40:13 GMT
<
[]
````

✅ Resultado: El microservicio responde con un `HTTP 200 OK`. Esto confirma que el archivo YAML ha configurado
correctamente no solo el despliegue, sino también la conectividad interna con la base de datos MySQL.

## 📋 Gestión Declarativa con `user-service-deployment.yml`: Escalando Instancias de Pods

### ¿Qué vamos a hacer?

En el apartado [Réplicas: Escalando Instancias de Pods en un Deployment](#) aprendimos a escalar Pods de forma
**imperativa** con el siguiente comando:

```bash
$ kubectl scale deployment d-user-service --replicas=3
```

Ahora veremos cómo lograr lo mismo de forma **declarativa**: simplemente modificando el archivo
`user-service-deployment.yml` y aplicando los cambios con `kubectl apply`.

### ✏️ Modificando el archivo `user-service-deployment.yml`

Incrementamos el valor de `replicas` a `5` en el archivo de configuración:
...
metadata:
  name: d-user-service
spec:
  replicas: 5           # <--- Definimos el nuevo estado deseado
  selector:
    matchLabels:
      app: d-user-service
  template:
...
Observemos la diferencia en el mensaje de respuesta en la consola según el estado del recurso:

| Mensaje      | Significado                                                                                                                                                                |
|--------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `created`    | Aparece cuando el recurso **no existe** y Kubernetes lo **crea desde cero.**                                                                                               |
| `configured` | Aparece cuando el recurso **ya existe** pero Kubernetes detecta cambios (en este caso, el número de réplicas) y actualiza el objeto vivo para que coincida con el archivo. |

### 🔍 Verificando las 5 réplicas

Listamos los Pods para confirmar que se crearon las 5 instancias:

````bash
$ kubectl get pods
NAME                              READY   STATUS    RESTARTS       AGE
d-mysql-6997b8d9fc-bkcr5          1/1     Running   14 (76m ago)   14d
d-user-service-59ffdb9645-9qfl7   1/1     Running   0              43m
d-user-service-59ffdb9645-b9fsg   1/1     Running   0              2m15s
d-user-service-59ffdb9645-jfbx6   1/1     Running   0              2m15s
d-user-service-59ffdb9645-lglx6   1/1     Running   0              2m15s
d-user-service-59ffdb9645-xwnjt   1/1     Running   0              2m15s
````

✅ El Pod `9qfl7` es el original con `AGE: 43m`, mientras que los 4 nuevos tienen `AGE: 2m15s` — todos creados a partir
de la modificación del archivo.

### 📡 Verificando el balanceo de carga en el Service

Lo más impresionante de Kubernetes es cómo los componentes se comunican entre sí.
El objeto `Service` `(svc-user-service)` detecta automáticamente los nuevos Pods gracias al `selector`.

Podemos verificar que el balanceador de carga ya reconoce las 5 IPs internas examinando los `Endpoints`:

```bash
$ kubectl describe service svc-user-service
Name:                     svc-user-service
Namespace:                default
Labels:                   <none>
Annotations:              <none>
...
Endpoints:                10.244.0.59:8001,10.244.0.61:8001,10.244.0.63:8001 + 2 more...
Session Affinity:         None
...
El campo `Endpoints` muestra las IPs internas de las 5 réplicas activas. El servicio las conoce
a todas y distribuirá el tráfico entre ellas automáticamente gracias al `Type: LoadBalancer`.

> 💡 El `+ 2 more...` indica que Kubernetes trunca la lista para no sobrecargar la salida, pero
> los 5 endpoints están registrados y activos.

### 📌 Conclusión

Este apartado demuestra el verdadero poder del `enfoque declarativo`: **basta con modificar un valor
en el archivo `YAML` y ejecutar `kubectl apply`** para que Kubernetes reconcilie el estado actual
del clúster con el estado deseado — sin necesidad de recordar comandos específicos ni gestionar
manualmente cada recurso.

```
Modificar YAML  →  kubectl apply  →  Kubernetes reconcilia  →  Estado deseado aplicado ✅
```

## 🐘 Escribiendo el Deployment Declarativo para PostgreSQL

### Contexto

Recordemos que cuando creamos el `Deployment` para MySQL lo hicimos generando automáticamente el
archivo `mysql-deployment.yml` a partir de un comando imperativo con `--dry-run=client`:
...
Este comando generó el archivo con ciertas configuraciones base definidas en la línea de comandos,
más otras configuraciones propias que Kubernetes agrega automáticamente al crear un `Deployment`.

### ✏️ Creando el archivo manualmente

Esta vez crearemos el archivo `postgres-deployment.yml` de forma completamente **manual**, escribiendo
cada configuración una a una. El objetivo es familiarizarnos con la estructura de un `Deployment`
y memorizarla progresivamente.

> 📚 **Recomendación:** Siempre es bueno apoyarse en la documentación oficial de Kubernetes,
> especialmente cuando configuramos recursos por primera vez. Para los `Deployments` puedes
> consultar: [kubernetes.io — Deployment](https://kubernetes.io/es/docs/concepts/workloads/controllers/deployment/)

### 📄 Archivo `postgres-deployment.yml`

Creamos el archivo en el directorio `kubernetes/deployments` con el siguiente contenido:
...
spec:
  ...
  selector:
    matchLabels:
      app: d-postgres     # Debe coincidir con la etiqueta del template
  template:
    metadata:
      labels:
        app: d-postgres
...
#### 🔍 **Desglose de la Estructura Manual**

Escribir un Deployment manualmente nos permite identificar claramente sus **tres capas principales:**

1. **Metadatos del Deployment (`metadata`):** Aquí definimos el nombre del objeto que verá Kubernetes (ej.
   `d-postgres`).
2. **Estrategia de Selección (`selector`):** Es crucial. Aquí le decimos al `Deployment`:
   *"Tú eres responsable de gestionar cualquier `Pod` que tenga la etiqueta `app: d-postgres`".*
3. **Plantilla del Pod (`template`):** Es el "molde" que usará Kubernetes para crear los contenedores. Incluye:
    - `Labels`: Las etiquetas que identifican al `Pod`.
    - `Spec del Contenedor`: La imagen (usamos la versión ligera alpine), el puerto y las variables de entorno
      necesarias para inicializar la base de datos.

#### 🔍 Explicación de las variables de entorno

| Variable            | Valor               | Descripción                                                                  |
|---------------------|---------------------|------------------------------------------------------------------------------|
| `POSTGRES_DB`       | `db_course_service` | Nombre de la base de datos que PostgreSQL creará automáticamente al iniciar. |
| `POSTGRES_USER`     | `postgres`          | Usuario administrador de la base de datos.                                   |
| `POSTGRES_PASSWORD` | `magadiflo`         | Contraseña del usuario administrador.                                        |

## 🔌 Escribiendo el Service Declarativo para PostgreSQL

### Contexto

Hasta este punto aún no hemos aplicado el `Deployment` de PostgreSQL que configuramos en el
apartado anterior — eso lo haremos más adelante junto con este `Service`, para mantener el orden
de nuestra documentación. **Primero escribimos todos los archivos, luego los aplicamos juntos.**

### ¿Qué vamos a hacer?

Crearemos un `Service` para exponer los `Pods` gestionados por el `Deployment` `d-postgres`
dentro del clúster. El objetivo es proporcionar un **punto de acceso estable** que permita al
microservicio de cursos comunicarse con la base de datos PostgreSQL.

Cuando el contenedor del microservicio de cursos arranque, necesitará conectarse a PostgreSQL.
Como ambos correrán en Pods distintos dentro del clúster, el `Service` actuará como intermediario
estable — sin importar cuántas veces se recree el Pod de PostgreSQL, la dirección de acceso
siempre será la misma: `svc-postgres:5432`.

### 📄 Archivo `postgres-service.yml`

Creamos el archivo en `kubernetes/services/postgres-service.yml` con el siguiente contenido:

```yaml
apiVersion: v1
kind: Service
metadata:
  name: svc-postgres # Nombre que actuará como DNS interno
spec:
  ports:
    - port: 5432
      protocol: TCP
      targetPort: 5432
  selector:
    app: d-postgres # Vincula este servicio con los pods del deployment d-postgres
  type: ClusterIP
````
...
| `metadata.name`    | `svc-postgres`    | Nombre del servicio. Otros Pods lo usarán como hostname DNS: `svc-postgres:5432`.                                                                     |
| `type`             | `ClusterIP`       | Expone el servicio únicamente dentro del clúster. No es accesible desde fuera. Es el tipopredeterminado y adecuado para comunicación entre `Pods`.    |
| `selector`         | `app: d-postgres` | Vincula el `Service` con los Pods que tengan la etiqueta `app: d-postgres`. Debe coincidir con la definida en el `Deployment` `d-postgres`.           |
| `ports.port`       | `5432`            | Puerto expuesto por el `Service` dentro del clúster.                                                                                                  |
| `ports.targetPort` | `5432`            | Puerto del contenedor al que el `Service` redirigirá el tráfico recibido. Es el puerto real donde el contenedor de PostgreSQL está escuchando (5432). |

#### 💡 Nota de Arquitectura: Comunicación entre Microservicios

Es fundamental entender que, gracias a este `Service`, la configuración de conexión en nuestro microservicio de cursos
(en su archivo de propiedades o variables de entorno) será:
...
Kubernetes se encargará de resolver el nombre `svc-postgres` a la IP interna del servicio y luego balancear el
tráfico hacia el Pod correspondiente.

## 🚀 Aplicando el Deployment y Service de PostgreSQL

Una vez que tenemos los archivos `postgres-deployment.yml` y `postgres-service.yml` correctamente
configurados, es momento de aplicarlos en el clúster.

### 📦 Aplicando el Deployment de PostgreSQL

Verificamos el estado actual de los `Deployments` antes de aplicar:

````bash
$ kubectl get deployments
NAME             READY   UP-TO-DATE   AVAILABLE   AGE
d-mysql          1/1     1            1           16d
d-user-service   5/5     5            5           47h
````

Aplicamos el archivo `postgres-deployment.yml`:
...
Verificamos que el `Deployment` `d-postgres` fue creado correctamente:

````bash
$ kubectl get deployments
NAME             READY   UP-TO-DATE   AVAILABLE   AGE
d-mysql          1/1     1            1           16d
d-postgres       1/1     1            1           29m
d-user-service   5/5     5            5           2d
````

Confirmamos que el Pod de PostgreSQL también fue creado:

````bash
$ kubectl get pods
NAME                              READY   STATUS    RESTARTS       AGE
d-mysql-6997b8d9fc-bkcr5          1/1     Running   15 (84m ago)   16d
d-postgres-5b58c85487-bqj95       1/1     Running   0              30m
d-user-service-59ffdb9645-9qfl7   1/1     Running   2 (83m ago)    2d
d-user-service-59ffdb9645-b9fsg   1/1     Running   2 (83m ago)    47h
d-user-service-59ffdb9645-jfbx6   1/1     Running   2 (83m ago)    47h
d-user-service-59ffdb9645-lglx6   1/1     Running   2 (83m ago)    47h
d-user-service-59ffdb9645-xwnjt   1/1     Running   2 (83m ago)    47h
````

### 🔌 Aplicando el Service de PostgreSQL

Aplicamos el archivo `postgres-service.yml`:

````bash
D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes-2026 (feature/section-14)
$ kubectl apply -f .\kubernetes\services\postgres-service.yml
service/svc-postgres created
````

Verificamos que el servicio `svc-postgres` fue creado correctamente:

````bash
$ kubectl get services
NAME               TYPE           CLUSTER-IP      EXTERNAL-IP   PORT(S)          AGE
kubernetes         ClusterIP      10.96.0.1       <none>        443/TCP          21d
svc-mysql          ClusterIP      10.100.3.217    <none>        3306/TCP         15d
svc-postgres       ClusterIP      10.102.167.67   <none>        5432/TCP         49s
svc-user-service   LoadBalancer   10.98.119.92    <pending>     8001:31169/TCP   10d
````

> 📌 **Nota:** Podríamos haber aplicado ambos archivos en un solo comando, pero lo hicimos uno
> por uno como parte del aprendizaje:
> ```bash
> $ kubectl apply -f .\kubernetes\deployments\postgres-deployment.yml -f .\kubernetes\services\postgres-service.yml
> ```

### 🗂️ Visión general del clúster con `kubectl get all`

Ahora que hemos creado varios recursos, podemos obtener una visión completa de todos los objetos
activos en el namespace actual con el siguiente comando:

````bash
$ kubectl get all
NAME                                  READY   STATUS    RESTARTS       AGE
pod/d-mysql-6997b8d9fc-bkcr5          1/1     Running   15 (86m ago)   16d
pod/d-postgres-5b58c85487-bqj95       1/1     Running   0              32m
pod/d-user-service-59ffdb9645-9qfl7   1/1     Running   2 (84m ago)    2d
pod/d-user-service-59ffdb9645-b9fsg   1/1     Running   2 (84m ago)    47h
pod/d-user-service-59ffdb9645-jfbx6   1/1     Running   2 (84m ago)    47h
pod/d-user-service-59ffdb9645-lglx6   1/1     Running   2 (84m ago)    47h
pod/d-user-service-59ffdb9645-xwnjt   1/1     Running   2 (84m ago)    47h

NAME                       TYPE           CLUSTER-IP      EXTERNAL-IP   PORT(S)          AGE
service/kubernetes         ClusterIP      10.96.0.1       <none>        443/TCP          21d
service/svc-mysql          ClusterIP      10.100.3.217    <none>        3306/TCP         15d
service/svc-postgres       ClusterIP      10.102.167.67   <none>        5432/TCP         24m
service/svc-user-service   LoadBalancer   10.98.119.92    <pending>     8001:31169/TCP   10d

NAME                             READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/d-mysql          1/1     1            1           16d
deployment.apps/d-postgres       1/1     1            1           32m
deployment.apps/d-user-service   5/5     5            5           2d

NAME                                        DESIRED   CURRENT   READY   AGE
replicaset.apps/d-mysql-6997b8d9fc          1         1         1       16d
replicaset.apps/d-postgres-5b58c85487       1         1         1       32m
replicaset.apps/d-user-service-59ffdb9645   5         5         5       2d
````

El comando `kubectl get all` lista en un solo output todos los recursos activos agrupados por tipo:

| Grupo              | Recursos listados                                      |
|--------------------|--------------------------------------------------------|
| `pod/`             | Todas las instancias en ejecución.                     |
| `service/`         | Todos los servicios de red expuestos.                  |
| `deployment.apps/` | Todos los Deployments y su estado de réplicas.         |
| `replicaset.apps/` | Todos los ReplicaSets gestionados por los Deployments. |

> 💡 `kubectl get all` es el comando ideal para tener una **foto completa del estado del clúster**
> en un solo vistazo, sin necesidad de ejecutar múltiples comandos por separado.

## 📋 Escribiendo de Forma Declarativa el Deployment y Service del Microservicio de Cursos

### ¿Qué vamos a hacer?

Crearemos los archivos `YAML` para el `Deployment` y el `Service` del microservicio de cursos.
Estos recursos definirán cómo se desplegará la aplicación dentro del clúster y cómo será accesible
para otros servicios y clientes externos.

### 📄 Archivo `course-service-deployment.yml`

Creamos el archivo en `kubernetes/deployments/course-service-deployment.yml` con la siguiente configuración:
...
| Variable            | Valor               | Descripción                                                                          |
|---------------------|---------------------|--------------------------------------------------------------------------------------|
| `CONTAINER_PORT`    | `8002`              | Puerto en el que el microservicio de cursos escucha dentro del contenedor.           |
| `DB_HOST`           | `svc-postgres`      | Hostname DNS del servicio de PostgreSQL dentro del clúster.                          |
| `DB_PORT`           | `5432`              | Puerto del servicio `svc-postgres`.                                                  |
| `DB_NAME`           | `db_course_service` | Nombre de la base de datos que usará el microservicio.                               |
| `DB_USERNAME`       | `postgres`          | Usuario de la base de datos PostgreSQL.                                              |
| `DB_PASSWORD`       | `magadiflo`         | Contraseña del usuario de la base de datos.                                          |
| `USER_SERVICE_HOST` | `svc-user-service`  | Hostname DNS del servicio de usuarios. Permite la comunicación entre microservicios. |
| `USER_SERVICE_PORT` | `8001`              | Puerto en el que el servicio `svc-user-service` escucha las peticiones entrantes.    |

> 💡 Observemos que `DB_HOST` usa `svc-postgres` y `USER_SERVICE_HOST` usa `svc-user-service`.
> Estos son los nombres de los `Services` que hemos creado anteriormente y que actúan como
> hostnames DNS estables dentro del clúster.

### 📄 Archivo `course-service-service.yml`

Creamos el archivo en `kubernetes/services/course-service-service.yml` con la siguiente configuración:
...
A diferencia del servicio de PostgreSQL que usaba `ClusterIP`, este servicio usa `LoadBalancer`,
lo que nos permitirá acceder al microservicio de cursos tanto **desde dentro del clúster** como
**desde nuestra máquina local** a través de Minikube.

> 💡 El `selector: app: d-course-service` vincula este `Service` con los Pods gestionados por
> el `Deployment` `d-course-service`, no con el `Deployment` en sí — tal como vimos anteriormente
> con el servicio de PostgreSQL.

## 🚀 Aplicando el Deployment y Service del Microservicio de Cursos

### ⚡ Aplicando ambos manifiestos en un solo comando

A diferencia de cuando aplicamos los recursos de PostgreSQL uno por uno, esta vez aplicamos ambos
archivos simultáneamente:

````bash
D:\programming\spring\01.udemy\02.andres_guzman\08.docker_kubernetes\docker-kubernetes-2026 (feature/section-14)
$ kubectl apply -f .\kubernetes\deployments\course-service-deployment.yml -f .\kubernetes\services\course-service-service.yml
deployment.apps/d-course-service created
service/svc-course-service created
````

Con un solo comando, Kubernetes creó el `Deployment` con sus Pods y el `Service` que los expone.

### 🔍 Verificando todos los recursos del clúster

Usamos `kubectl get all` para obtener una visión completa del estado actual del clúster:

```bash
$ kubectl get all
NAME                                    READY   STATUS    RESTARTS        AGE
pod/d-course-service-5cf78fdcd7-5nfvv   1/1     Running   0               117s
pod/d-mysql-6997b8d9fc-bkcr5            1/1     Running   15 (143m ago)   16d
pod/d-postgres-5b58c85487-bqj95         1/1     Running   0               89m
pod/d-user-service-59ffdb9645-9qfl7     1/1     Running   2 (142m ago)    2d1h
pod/d-user-service-59ffdb9645-b9fsg     1/1     Running   2 (142m ago)    2d
pod/d-user-service-59ffdb9645-jfbx6     1/1     Running   2 (142m ago)    2d
pod/d-user-service-59ffdb9645-lglx6     1/1     Running   2 (142m ago)    2d
pod/d-user-service-59ffdb9645-xwnjt     1/1     Running   2 (142m ago)    2d

NAME                         TYPE           CLUSTER-IP      EXTERNAL-IP   PORT(S)          AGE
service/kubernetes           ClusterIP      10.96.0.1       <none>        443/TCP          21d
service/svc-course-service   LoadBalancer   10.104.20.240   <pending>     8002:31051/TCP   117s
service/svc-mysql            ClusterIP      10.100.3.217    <none>        3306/TCP         16d
service/svc-postgres         ClusterIP      10.102.167.67   <none>        5432/TCP         81m
service/svc-user-service     LoadBalancer   10.98.119.92    <pending>     8001:31169/TCP   10d

NAME                               READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/d-course-service   1/1     1            1           117s
deployment.apps/d-mysql            1/1     1            1           16d
deployment.apps/d-postgres         1/1     1            1           89m
deployment.apps/d-user-service     5/5     5            5           2d1h

NAME                                          DESIRED   CURRENT   READY   AGE
replicaset.apps/d-course-service-5cf78fdcd7   1         1         1       117s
replicaset.apps/d-mysql-6997b8d9fc            1         1         1       16d
replicaset.apps/d-postgres-5b58c85487         1         1         1       89m
replicaset.apps/d-user-service-59ffdb9645     5         5         5       2d1h
````

Confirmamos que todos los recursos del microservicio de cursos fueron creados exitosamente:

| Recurso      | Nombre                              | Estado                            |
|--------------|-------------------------------------|-----------------------------------|
| `Pod`        | `d-course-service-5cf78fdcd7-5nfvv` | ✅ `Running` con `RESTARTS: 0`     |
| `Service`    | `svc-course-service`                | ✅ `LoadBalancer` en puerto `8002` |
| `Deployment` | `d-course-service`                  | ✅ `1/1` réplica disponible        |
| `ReplicaSet` | `d-course-service-5cf78fdcd7`       | ✅ `1/1` listo                     |

## 🧪 Pruebas de Conectividad HTTP: Verificación de Despliegue y Comunicación entre Servicios

### 🔗 1. Obteniendo la URL del servicio de cursos

Generamos la URL dinámica del servicio `svc-course-service`:

````bash
$ minikube service svc-course-service --url
http://127.0.0.1:49192
! Because you are using a Docker driver on windows, the terminal needs to be open to run it.
````

> ⚠️ **Recordatorio:** Como usamos Minikube con el driver Docker, esta URL estará disponible
> únicamente mientras la terminal permanezca abierta, ya que Minikube crea un túnel local mediante
> `port forwarding` para exponer el servicio fuera del clúster.

### 📋 2. Realizando peticiones
...
```bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"name\": \"Docker\"}" http://127.0.0.1:49192/api/v1/courses | jq
>
< HTTP/1.1 201
< Location: http://127.0.0.1:49192/api/v1/courses/1
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 08 May 2026 23:04:27 GMT
<
{
  "id": 1,
  "name": "Docker"
...
`HTTP 201 Created` — el curso fue registrado exitosamente. El header `Location` indica la URL
del recurso recién creado: `/api/v1/courses/1`.

#### 👤 Registrando un usuario y asignándolo al curso

Con el curso creado, registramos un usuario y lo asignamos directamente al curso `1`. Esta
petición nos permitirá verificar la **comunicación entre el microservicio de cursos y el de
usuarios**:

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"name\": \"Briela\", \"email\": \"briela@gmail.com\", \"password\": \"123456\"}" http://127.0.0.1:49192/api/v1/courses/1/users | jq
>
< HTTP/1.1 201
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 08 May 2026 23:06:12 GMT
<
{
  "id": 1,
  "name": "Briela",
...
`HTTP 201 Created` — el usuario fue registrado y asociado al curso exitosamente.

#### 🔍 Verificando la relación curso-usuario

Consultamos el detalle del curso para confirmar que el usuario quedó correctamente asociado:

```bash
$ curl -v -G --data "loadRelations=true" http://127.0.0.1:49192/api/v1/courses/1 | jq
>
< HTTP/1.1 200
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Fri, 08 May 2026 23:07:31 GMT
<
{
  "id": 1,
  "name": "Docker",
...
`HTTP 200 OK` — el curso `Docker` muestra correctamente al usuario `Briela` asociado.

### 📌 Conclusión

Estas pruebas confirman que toda la cadena funciona correctamente de extremo a extremo:
...
|-------------------------------------|-----------------------------------------------------------------------------------------|
| 🚀 **Despliegue**                   | El microservicio de cursos fue desplegado correctamente y expone su API.                |
| 🔗 **Comunicación entre servicios** | `course-service` se comunicó exitosamente con `user-service` para registrar el usuario. |
| 🗄️ **Persistencia**                | Los datos fueron almacenados correctamente en PostgreSQL y MySQL respectivamente.       |
| 🌐 **Acceso externo**               | El servicio `svc-course-service` de tipo `LoadBalancer` fue accesible desde Minikube.   |
